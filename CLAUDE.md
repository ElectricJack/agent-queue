...
- **OpenAPI spec:** `openapi.json` is a committed artifact both clients are generated from. `src/api/spec.py` builds it offline (no daemon) — regenerate with `./scripts/regenerate-api-client.sh --offline` and then `./scripts/regenerate-ts-client.sh --from-file` after **any** change to `src/api/models` or a codegen router. `tests/test_api_client_contract.py::test_committed_openapi_json_matches_the_live_app_surface` fails when the committed spec drifts from what `create_app()` serves. The Python generator is pinned exactly (`GENERATOR_VERSION` in the script, `openapi-python-client==` in the `dev` extra, kept in sync by `test_generator_version_pin_agrees_between_the_script_and_the_dev_extra`) — the whole `packages/aq-client/` tree, generated `README.md` included, is a function of the generator version, so an unpinned generator makes regeneration non-idempotent.
