 ...
 *  testing the payload the daemon actually serves. `fixtures.ts` next door is
 *  still hand-authored; anything a card claims about the backend belongs here
 *  instead, because a hand-authored node can agree with a component while
 *  disagreeing with the projector — which is how the compact card came to omit
 *  its key inputs and its output binding for a whole package (`solid-harbor.49`
 *  pass 2).
 *
