 ...
 *
 *  Still hand-authored, and therefore not evidence about the backend: the real
 *  `project_graph` output is checked in next door as `graph.fixture.json`,
 *  regenerated by `python -m tests.playbook_v2_helpers` and asserted byte for
 *  byte by `tests/test_playbook_v2_api_dtos.py`. `StepNodeCard.test.tsx` reads
 *  that projection through `projected.ts` instead of this file, because the
 *  badges and rows below were chosen to match the components rather than
 *  projected — which is how the compact card came to omit its key inputs and
 *  its output binding for a whole package (`solid-harbor.49` pass 2). The
 *  remaining suites still read this file for surfaces the projection fixture
 *  does not cover (redacted values, a resolved AI routing block, run
 *  overlays); moving each of them over is the rest of §16.10 deviation 1.
 *  Every object is typed against the generated client, so a DTO change breaks
 *  the typecheck rather than the runtime. */
