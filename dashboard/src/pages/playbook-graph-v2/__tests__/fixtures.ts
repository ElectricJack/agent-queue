 ...
 *
 *  Still hand-authored: `project_graph` has since landed and its real output
 *  is checked in next door as `graph.fixture.json`, regenerated by
 *  `python -m tests.playbook_v2_helpers` and asserted byte for byte by
 *  `tests/test_playbook_v2_api_dtos.py`. Replacing this file's contents with
 *  that JSON is the remaining half of the plan's §16.10 deviation 1 and is
 *  tracked separately, because the component tests below assert values that
 *  were chosen here rather than projected. Every object is typed against the
 *  generated client, so a DTO change breaks the typecheck rather than the
 *  runtime. */
