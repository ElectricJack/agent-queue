...
export default function LeftRail() {
  ...
  const { projectId, tab, isWorkspace } = projectNavigation(location.pathname);
    ...
    <aside className="col-start-1 row-start-2 flex h-full w-64 shrink-0 lg:w-72 flex-col overflow-hidden border-r border-gray-800 bg-gray-900">
      <nav ref={navRef} className="flex-1 space-y-6 overflow-y-auto p-3">
        <div className="space-y-0.5">
          <Link to={workspaceHref(projectId, tab, location.search)} data-listnav="1"
        ...
        <div>
          ...
          <div className="space-y-0.5">
            <Link to={workspaceHref(null, tab, location.search)} data-listnav="1"
            ...
            {(projects ?? []).map((p) => (
              <Link
                key={p.id}
                to={workspaceHref(p.id, tab, location.search)}
                data-listnav="1"
