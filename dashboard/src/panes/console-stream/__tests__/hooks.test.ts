...
describe("useConsoleStream", () => {
  ...
  it("reconnects a slow managed viewer from its delivered byte cursor", async () => {
    ...
    const es = FakeEventSource.instances[0]!;
    act(() => {
      es.emit({ type: "line", seq: 123, text: "delivered", ts: 1, after: 100, next: 123 });
      // The server's marker can name an older queued cursor; it is not output.
      es.emit({
        type: "gap", seq: 100, after: 100, next: 100, ts: 1,
    ...
    expect(result.current.lines.map((line) => line.text)).toEqual(["delivered"]);
