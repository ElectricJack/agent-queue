...
- `priority` (optional)
- `project_id` (optional): Move a standalone root-leaf task to a different project. The
  move is refused while the task has a parent, children, or an active hierarchy/train
  branch origin. Otherwise it also rebinds `repo_id` unless the destination owns the
  task's repository: it becomes the destination's `integration_repository_id` in a
