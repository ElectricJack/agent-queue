...
`effective_requirements` is **unchanged** (canonical order preserved, still pure relative to DB state). `acquire_for_task` keeps its all-or-nothing rollback and gains one optional argument, `preferred_workspaces: dict[kind_id, workspace_id]` — the caller-computed §3.4 branch-affinity hint, applied only to requirements that carry no explicit `preferred_workspace_id` and fed into the existing `prefer_workspace_id` parameter, which already falls back to first-unlocked when the named row is busy. The hint itself is built by `WorkspaceMixin._slot_branch_affinity` from `WorktreeSlotManager.find_slot_holding_branch` (one `git worktree list --porcelain` per acquisition, failures degrading to "no preference"). `acquire_one_unlocked` (line 267) gains mode awareness: when the resolved kind has `mode='worktree'` (and `worktrees.enabled`), the candidate filter adds `slot_index IS NOT NULL` (bases excluded) and, when fewer free slots than the cap exist, the caller (`_prepare_workspace`) invokes `ensure_slots` and retries once. Both dialect strategies (SQLite `BEGIN IMMEDIATE` + rowcount, PG `FOR UPDATE SKIP LOCKED`) apply to slot rows unmodified.
...
- [x] **Phase 3 — Merge slot & pipeline.** `merge_slots` queries, `_phase_integrate`, verify-phase expectation changes, conflict → `rejection_reason` + files, events. `_phase_verify`'s auto-merge is now explicitly skipped under worktree mode (`git_ops.py:629–637` computes `is_worktree_task` and gates the remediation on it) rather than relying on git's refusal of `checkout <default_branch>` inside the slot.
- [x] **Phase 4 — Reaper & adoption.** Cascade steps 7d/7e, `adopt_existing` in recovery, `_recover_stale_state` worktree-deletion removal, branch pruning, `--reset` escape hatch.
- [ ] **Phase 5 — Surface.** `workspace list` annotations, `workspace_doctor`, `workspace_reap` are done; `worktree.*`/`merge.*` events are registered with the validator but not yet documented in `docs/specs/event-bus.md`, and there is no dashboard workspace view consuming this data.
- [ ] **Phase 6 — Flag flip & cleanup.** `worktrees.enabled` now defaults to `true` (flag flip done); the legacy `_cleanup_worktree_workspace` path is not yet deleted — still referenced for legacy non-slot `WORKTREE` rows.
