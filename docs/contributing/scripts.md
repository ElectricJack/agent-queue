...
|---|---|---|---|
| [`regenerate-api-client.sh`](../../scripts/regenerate-api-client.sh) | Rebuild `openapi.json` and the typed Python client. | `--offline` (build the spec from the checkout), `--from-file`, or no argument (fetch from a daemon at `AGENT_QUEUE_API_URL`); optionally `--install`. Anything else is a usage error (exit 2). | Rewrites `openapi.json`, deletes and regenerates `packages/aq-client/`, rewrites `scripts/aq-client-boilerplate.sha256` — tracked files only. It touches the Python environment only with `--install`, which is refused in a worker session and refuses to re-point an install that resolves to another tree ([why](codegen.md#the-script-leaves-the-environment-alone)). Refuses to run on the wrong generator version or without `ruff`. |
