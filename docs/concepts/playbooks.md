...
**Shipped required defaults.** `default-assignment-routing` and `provider-usage-probe` are the IDs in `REQUIRED_SYSTEM_PLAYBOOK_IDS`; `blocked-task-escalation` is a shared default that is activated once but is not a readiness requirement. Every start seeds each shipped reviewed bundle into `vault/reviewed-playbooks/<id>/` and *refreshes* a vault copy that differs from what ships, moving the old directory aside to `<id>.bak-<epoch>` — those directories are daemon-owned, because a reviewed artifact freezes the capability fingerprint of every profile it names and a copy seeded by an older release can never validate once those grants move. The reconciler then creates a missing system activation, keeps a healthy or disabled one exactly as it is, and re-points an *enabled* one that no longer validates at the freshly imported shipped artifact. Readiness (`/health`, `/ready`) is recomputed from live activation health on every read and after every activation write, not frozen at startup. An inactive required router retains `task.route_needed` events rather than dropping them. [Required policy handling](../../src/playbooks/required.py) is the source of truth.
...
| Activation is `stale_contract` | A referenced command contract or profile capability fingerprint changed after compilation. | For a playbook AQ ships, restart the daemon (or `aq doctor --check playbooks.reviewed_bundles --fix`): the shipped bundle is re-seeded and an enabled system activation is re-pointed at it. If that import fails too, the readiness diagnostic names the contract or profile that moved — usually a vault profile whose grants differ from the shipped one. For local policy, re-propose/review/import against current contracts, inspect `artifact-diff`, and activate the compatible artifact. |
