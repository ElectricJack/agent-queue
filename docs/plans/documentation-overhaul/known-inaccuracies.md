...
| Two publication formats — **resolved** | `mkdocs.yml` configured a Material site at `electricjack.github.io/agent-queue/`, deployed by `.github/workflows/docs.yml` on every push to `main` touching `docs/**`, while this overhaul publishes GitHub-rendered Markdown with relative links. 49 of the 77 pages in that site's `nav:` block did not exist, so the deploy served a partly-404 mirror. Decision: one publication format. `mkdocs.yml`, `.github/workflows/docs.yml`, `scripts/generate-docs.sh` and `pyproject.toml`'s `docs` extra are deleted; the record is [CI § there is no documentation build](../../contributing/ci.md#there-is-no-documentation-build). The `github-pages` environment is a repository setting and still has to be disabled by hand. | `legacy` |
