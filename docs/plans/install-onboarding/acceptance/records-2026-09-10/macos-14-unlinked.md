...
- Recorded: `2026-09-10T07:25:00+00:00`
- Runner: `GitHub Actions 1000007693`
- macOS: `14.8.9` (`ProductName:		macOS / ProductVersion:		14.8.9 / BuildVersion:		23J631`)
- Architecture: `arm64` (`arch` reports `arm64`)
- Homebrew: `Homebrew 6.0.20
Homebrew/homebrew-core (git revision ae26c0a85ae; last commit 2026-08-31)
Homebrew/homebrew-cask (git revision 052c719b067; last commit 2026-08-30)` at `/opt/homebrew`
- Xcode CLT: `/Applications/Xcode_15.4.app/Contents/Developer`
...
| --- | --- | --- |
| `host` | pass | macOS 14.8.9 on arm64 |
...
- `sysctl -n hw.model` → exit `0` (0.0s)
- `xcode-select -p` → exit `0` (0.0s)
- `brew --version` → exit `0` (0.09s)
- `brew --prefix` → exit `0` (0.01s)
- `git --version` → exit `0` (0.0s)
- `/Library/Frameworks/Python.framework/Versions/3.12/bin/python3 -V` → exit `0` (0.01s)

### `matrix` — Supported-platform verdict

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --list-steps --json` → exit `0` (1.85s)
- `/Library/Frameworks/Python.framework/Versions/3.12/bin/python3 -c import json;from src.install import describe_host;print(json.dumps(describe_host().to_dict()))` → exit `0` (0.13s)

### `plan` — Dry run

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --dry-run --json --with postgres-managed --with daemon` → exit `0` (2.13s)

### `install` — Unattended install to a ready daemon

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --with postgres-managed --with daemon` → exit `0` (24.06s)

### `rerun` — Rerun (idempotence)

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --with postgres-managed --with daemon` → exit `0` (3.19s)

### `repair` — Repair

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --repair --with postgres-managed --with daemon` → exit `0` (2.79s)

### `upgrade` — Upgrade

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --upgrade --with postgres-managed --with daemon` → exit `0` (2.76s)

### `restart-from` — Resume one branch (--restart-from)

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --restart-from config.defaults --with postgres-managed --with daemon` → exit `0` (3.41s)

### `daemon` — Daemon and dashboard answer

- `curl -sS -o /dev/null -w %{http_code} http://127.0.0.1:8081/health` → exit `0` (0.03s)
- `curl -sS -o /dev/null -w %{http_code} http://127.0.0.1:8081/dashboard` → exit `0` (0.03s)
- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq status` → exit `0` (2.15s)

### `database` — PostgreSQL under brew services

- `brew services list` → exit `0` (0.82s)
- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq doctor --check database.connection --json` → exit `2` (1.76s)

### `postgres-client` — Where psql comes from

- `brew --prefix` → exit `0` (0.02s)
- `brew --prefix postgresql@17` → exit `0` (0.02s)
- `/bin/sh -c command -v psql || true` → exit `0` (0.0s)
- `/bin/sh -c brew list --versions | grep -i postgres || true` → exit `0` (0.51s)

### `first-task` — First project and task

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq project list-roots --json` → exit `0` (1.9s)

### `uninstall-plan` — Uninstall plan (dry run)

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq uninstall --dry-run --json` → exit `0` (2.01s)
