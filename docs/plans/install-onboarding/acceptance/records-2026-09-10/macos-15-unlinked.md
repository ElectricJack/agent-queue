...
- Recorded: `2026-09-10T07:24:43+00:00`
...
- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --list-steps --json` → exit `0` (1.69s)
- `/Library/Frameworks/Python.framework/Versions/3.12/bin/python3 -c import json;from src.install import describe_host;print(json.dumps(describe_host().to_dict()))` → exit `0` (0.15s)

### `plan` — Dry run

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --dry-run --json --with postgres-managed --with daemon` → exit `0` (1.81s)

### `install` — Unattended install to a ready daemon

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --with postgres-managed --with daemon` → exit `0` (20.29s)

### `rerun` — Rerun (idempotence)

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --with postgres-managed --with daemon` → exit `0` (3.47s)

### `repair` — Repair

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --repair --with postgres-managed --with daemon` → exit `0` (3.8s)

### `upgrade` — Upgrade

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --upgrade --with postgres-managed --with daemon` → exit `0` (4.67s)

### `restart-from` — Resume one branch (--restart-from)

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --restart-from config.defaults --with postgres-managed --with daemon` → exit `0` (4.21s)
...
- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq status` → exit `0` (1.63s)

### `database` — PostgreSQL under brew services

- `brew services list` → exit `0` (0.53s)
- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq doctor --check database.connection --json` → exit `2` (1.69s)
...
- `/bin/sh -c brew list --versions | grep -i postgres || true` → exit `0` (0.43s)

### `first-task` — First project and task

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq project list-roots --json` → exit `0` (1.54s)

### `uninstall-plan` — Uninstall plan (dry run)

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq uninstall --dry-run --json` → exit `0` (1.31s)
