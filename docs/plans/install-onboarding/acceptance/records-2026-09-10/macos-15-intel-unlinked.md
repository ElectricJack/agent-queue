...
- Recorded: `2026-09-10T07:29:19+00:00`
...
- `arch` → exit `0` (0.01s)
- `sysctl -n sysctl.proc_translated` → exit `1` (0.01s)
- `sysctl -n hw.optional.arm64` → exit `1` (0.01s)
- `sysctl -n hw.model` → exit `0` (0.0s)
- `xcode-select -p` → exit `0` (0.01s)
- `brew --version` → exit `0` (0.22s)
- `brew --prefix` → exit `0` (0.03s)
...
- `/Users/runner/hostedtoolcache/Python/3.12.10/x64/bin/aq install --list-steps --json` → exit `0` (3.54s)
- `/Library/Frameworks/Python.framework/Versions/3.12/bin/python3 -c import json;from src.install import describe_host;print(json.dumps(describe_host().to_dict()))` → exit `0` (0.3s)

### `plan` — Dry run

- `/Users/runner/hostedtoolcache/Python/3.12.10/x64/bin/aq install --non-interactive --dry-run --json --with postgres-managed --with daemon` → exit `0` (3.82s)

### `install` — Unattended install to a ready daemon

- `/Users/runner/hostedtoolcache/Python/3.12.10/x64/bin/aq install --non-interactive --yes --json --with postgres-managed --with daemon` → exit `0` (48.21s)

### `rerun` — Rerun (idempotence)

- `/Users/runner/hostedtoolcache/Python/3.12.10/x64/bin/aq install --non-interactive --yes --json --with postgres-managed --with daemon` → exit `0` (7.71s)

### `repair` — Repair

- `/Users/runner/hostedtoolcache/Python/3.12.10/x64/bin/aq install --non-interactive --yes --json --repair --with postgres-managed --with daemon` → exit `0` (7.07s)

### `upgrade` — Upgrade

- `/Users/runner/hostedtoolcache/Python/3.12.10/x64/bin/aq install --non-interactive --yes --json --upgrade --with postgres-managed --with daemon` → exit `0` (6.81s)

### `restart-from` — Resume one branch (--restart-from)

- `/Users/runner/hostedtoolcache/Python/3.12.10/x64/bin/aq install --non-interactive --yes --json --restart-from config.defaults --with postgres-managed --with daemon` → exit `0` (9.91s)

### `daemon` — Daemon and dashboard answer

- `curl -sS -o /dev/null -w %{http_code} http://127.0.0.1:8081/health` → exit `0` (0.07s)
- `curl -sS -o /dev/null -w %{http_code} http://127.0.0.1:8081/dashboard` → exit `0` (0.05s)
- `/Users/runner/hostedtoolcache/Python/3.12.10/x64/bin/aq status` → exit `0` (3.94s)

### `database` — PostgreSQL under brew services

- `brew services list` → exit `0` (1.15s)
- `/Users/runner/hostedtoolcache/Python/3.12.10/x64/bin/aq doctor --check database.connection --json` → exit `2` (3.26s)

### `postgres-client` — Where psql comes from

- `brew --prefix` → exit `0` (0.04s)
- `brew --prefix postgresql@17` → exit `0` (0.05s)
- `/bin/sh -c command -v psql || true` → exit `0` (0.01s)
- `/bin/sh -c brew list --versions | grep -i postgres || true` → exit `0` (1.04s)

### `first-task` — First project and task

- `/Users/runner/hostedtoolcache/Python/3.12.10/x64/bin/aq project list-roots --json` → exit `0` (4.95s)

### `uninstall-plan` — Uninstall plan (dry run)

- `/Users/runner/hostedtoolcache/Python/3.12.10/x64/bin/aq uninstall --dry-run --json` → exit `0` (3.73s)
