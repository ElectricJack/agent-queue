...
- Recorded: `2026-09-10T07:24:38+00:00`
- Runner: `GitHub Actions 1000007694`
...
- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --list-steps --json` → exit `0` (1.76s)
- `/Library/Frameworks/Python.framework/Versions/3.12/bin/python3 -c import json;from src.install import describe_host;print(json.dumps(describe_host().to_dict()))` → exit `0` (0.16s)

### `plan` — Dry run

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --dry-run --json --with postgres-managed --with daemon` → exit `0` (1.68s)

### `install` — Unattended install to a ready daemon

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --with postgres-managed --with daemon` → exit `0` (18.28s)

### `rerun` — Rerun (idempotence)

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --with postgres-managed --with daemon` → exit `0` (2.72s)

### `repair` — Repair

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --repair --with postgres-managed --with daemon` → exit `0` (2.91s)

### `upgrade` — Upgrade

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --upgrade --with postgres-managed --with daemon` → exit `0` (3.22s)

### `restart-from` — Resume one branch (--restart-from)

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq install --non-interactive --yes --json --restart-from config.defaults --with postgres-managed --with daemon` → exit `0` (2.94s)

### `daemon` — Daemon and dashboard answer

- `curl -sS -o /dev/null -w %{http_code} http://127.0.0.1:8081/health` → exit `0` (0.02s)
- `curl -sS -o /dev/null -w %{http_code} http://127.0.0.1:8081/dashboard` → exit `0` (0.01s)
- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq status` → exit `0` (1.6s)

### `database` — PostgreSQL under brew services

- `brew services list` → exit `0` (0.67s)
- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq doctor --check database.connection --json` → exit `2` (1.71s)
...
- `/bin/sh -c brew list --versions | grep -i postgres || true` → exit `0` (0.47s)

### `first-task` — First project and task

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq project list-roots --json` → exit `0` (1.73s)

### `uninstall-plan` — Uninstall plan (dry run)

- `/Users/runner/hostedtoolcache/Python/3.12.10/arm64/bin/aq uninstall --dry-run --json` → exit `0` (1.85s)
