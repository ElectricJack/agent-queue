...
- **New profile grants need an operator step on an existing install** (final review I5). `ensure_default_profiles` is write-if-absent, so the `task_subtask_*` / `phase_*` grants the shipped profiles gained reach a fresh install and no upgraded one. The rollout is documented rather than automatic — overwriting a vault copy would discard operator edits — in `docs/guides/worker-pools.md` §7b under "A shipped profile that gained a new grant": `aq doctor --check profiles.system_drift` reports which copies diverge, `aq agent profile-reseed --profile-id <id>` rewrites one from the shipped default after writing a `.bak-<epoch>`. As a guard, prime omits the `## Subtasks` block's "report progress" line for a profile whose capability policy lacks `task_subtask_update` (`profile_allows_command`), so an un-reseeded worker sees the checklist instead of being told to run a command its own policy denies.
