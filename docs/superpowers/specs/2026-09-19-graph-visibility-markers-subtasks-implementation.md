...
- **A phase gates behind every earlier open phase, not only its predecessor** (final review I4). One edge per phase meant deleting an abandoned empty phase 2 dropped the only edge phase 3 had and released it while phase 1 was still open. `phase_create` now writes a `blocks` edge onto every earlier sibling phase that has not COMPLETED; the response keeps `blocked_by` (the immediate predecessor) and adds `blocked_by_all` (the gate set).
...
- **`parent_key` is refused in hierarchy/train projects, and the CI main sentinel does not use it** (final review C1). In those modes the child is filed through `file_prepared_child_on`: based on the standing container's checkpoint — main as of the *container's* creation — and delivered to the container's `aq/<id>` branch, so it reaches the default branch only when the container settles, which one FAILED or BLOCKED sibling prevents indefinitely. The plan's shipped caller was the sentinel, whose whole job is to repair main's red head, so the argument is refused before any write (`hierarchy.parent_key_unsupported_mode`, both `create_task` and `ensure_task`) and the sentinel playbook creates its repair at the project root. The mechanism is unchanged for non-hierarchical projects. It also validates the key itself (1–64 characters, `[a-z0-9][a-z0-9_-]*` → `hierarchy.parent_key_invalid`) and reserves the `parent:` dedup-key prefix against caller-supplied keys (`hierarchy.reserved_dedup_key`).
...
- **New profile grants need an operator step on an existing install** (final review I5). `ensure_default_profiles` is write-if-absent, so the `task_subtask_*` / `phase_*` grants the shipped profiles gained reach a fresh install and no upgraded one. As a guard, prime omits the `## Subtasks` block's "report progress" line for a profile whose capability policy lacks `task_subtask_update` (`profile_allows_command`), so an un-reseeded worker sees the checklist instead of being told to run a command its own policy denies.
- **Task 14 closed the gap I5 above left as an operator-documented manual step, with detection *and* an edit-preserving repair.** `profiles.system_drift` (`aq doctor --check profiles.system_drift`) previously compared only `## Config` and section names, so it was blind to grant lists; `diff_profile`/`scan_profile_drift` (`src/profiles/drift.py`) now also report `missing_grants` per capability namespace (`harness_tools`/`aq_commands`/`plugin_tools` — the exact names a vault copy lacks that the shipped default grants; extra vault grants are never drift), and the check's detail names `--grants-only` whenever any profile has missing grants. The repair is `merge_profile_grants`, exposed as `aq agent profile-reseed --profile-id <id> --grants-only`: it edits the vault file's `## Capabilities` fenced JSON block textually — appending only the missing names to each namespace list — so `## Config` (including an operator's `harness: codex`), every other section, and any grant the operator added themselves survive byte-for-byte; a `.bak-<epoch>` is written first, and the merged text is validated with `parse_profile` before the file is replaced (a failure to parse leaves no write and no backup). A vault copy still on the legacy `## Tools` block (no `## Capabilities` at all) is refused with a pointer to a full reseed. A full `aq agent profile-reseed --profile-id <id>` (no flag) remains for that case and for any divergence an operator would rather just overwrite. See `docs/guides/worker-pools.md` §7b, "A shipped profile that gained a new grant", for the operator-facing walkthrough.
...
- **Graph**: `auto_expand`/`expanded_applied` and `active_expansion()` landed as specified; the client persisted the applied set once per project via `expanded_initialised` on the `command_center_project_view` dashboard-state document. **Superseded (2026-09-20, Task 28).** The operator ruled that the graph must never expand a container in place at all — every container is a compact tile at every zoom and you ENTER it, recursively, with breadcrumbs back out. The client half of the inline active expansion is gone with it (the `auto_expand` request, `applyExpandedResult`/`hasStoredExpansion`/`expanded_initialised` reads, the chevron toggles, the zoom-derived `max_depth`, and the "Focus active" toolbar button); the canvas always asks for `expanded: []` and picks its scope with `root`. **One documented exception:** under an active search query or status filter the server still force-opens the ancestors of every match (`forced_expansion_for`), so containers holding results do draw expanded — a search result has to be visible where it lives. The jump handler respects that: a hit already drawn in the current scope is panned to and highlighted, and only a hit that is not drawn enters its container. The SERVER keeps `auto_expand`/`active_expansion()`/`expanded` and the dashboard-state fields — other callers may still use them, and the fields are preserved verbatim on every write — but nothing in the dashboard reads or writes them, so a stored expansion has no effect. Server changes: a focused request no longer forces `variant="all"` — entering a container keeps the variant the root view uses (so "Show completed" still means what it says inside one) and is promoted only when the entered container is a stub in, or missing from, the active layout (`_variant_for_scope`). Because that condition is the layout's and not the task's status — a container every one of whose descendants has finished is stubbed whatever its own status — `TilesResponse`/`ListResponse` report `variant_applied` rather than leaving the client to infer it, and `ListRequest`/`LocateRequest` gained the same `root` as `tiles` so the phone view pages the entered scope and a located hit's box comes from the geometry the focused canvas draws. `ProgressBar` is a shared component across containers, phases and subtask-bearing task cards. `discovered-from` edges render dashed with no arrowhead and are excluded from the layout engine's layering inputs, matching the plan's requirement that they stay pure annotation.
