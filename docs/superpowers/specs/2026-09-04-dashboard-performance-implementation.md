...
- Version-less aggregate refresh: D2's aggregates-only publish still bumps `layout_version`, so every completion still cold-starts the tiles geometry cache for every viewer (investigation §3, second paragraph). A separate aggregates version, or reading `agg_*` live in the tiles response, would let a pure status flip leave the geometry cache warm.
- Active-variant hole compaction: D3 leaves a finished leaf's slot empty until a sibling is created, moved or reopened, or a tidy job runs (design §4.6). A project whose tasks complete without new siblings accumulates holes; a per-container hole-count threshold that schedules an ordinary re-lay (in the background, after D1) would bound that without paying 1.4 s per completion.
