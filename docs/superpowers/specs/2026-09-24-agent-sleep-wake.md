...
| `ChildTaskReconciler` | `src/playbooks/engine.py:3651`, ticked via `reconcile_playbook_child_tasks` (`src/commands/playbook_commands.py:685`) | scans durable state | A *scan*, not a bus subscriber, because "a task reaches a terminal status from some thirty call sites … several of which emit nothing". `settled_child_waits` (`src/database/queries/playbook_run_queries.py:1402`) joins waits to `tasks`/`archived_tasks`. Idempotent: the wait is cleared in the boundary that takes the edge. |
