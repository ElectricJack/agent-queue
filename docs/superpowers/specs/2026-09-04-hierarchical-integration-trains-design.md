...
proof required of collectors, verifiers, and repair delegates — §9.1–§9.2 depend on a writer the
daemon can stop and whose workspace it can retain, which is why a repair delegate is never given
to a pool worker.
