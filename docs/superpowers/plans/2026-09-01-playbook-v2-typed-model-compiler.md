...
10. **Amendments applied.** Record here, with a date, every amendment made under §3.2.1's stop-and-amend rule.
   ...
   - *2026-09-02* — **Commits 1–2 landed** (`definition.py`, `expressions.py`, `validation.py`, the three suites and the fixture set) on `origin/main` `e0ae15eb`. §3.2.1's checklist was re-run and reports a materially different world from the one this plan was drafted in; every deviation below is implemented as written.

     1. **Packages 0, 1, 3 and 5 have all merged.** §3.2's "Package 1 has NOT landed" is obsolete: `src/commands/contracts/` exists and `CONTRACTS` carries ten built-in contracts, including the golden fixture's `ensure_task`, `gate_create` and `list_tasks`. §3.3's two "degraded modes" are therefore no longer degraded — both adapters resolve for real — but the `ContractLookup` / `ProfileLookup` protocols stay, because they are what lets the suites build a contract and capability lattice without a vault.
     2. **Package 1's symbol names differ from §3.2's assumption.** There is no `get_contract`; the registry is `ContractRegistry.get(name) -> CommandRegistration | None` with the contract at `.contract` and its execution shape at `.contract.execution` (`args_model`, `result_model`, `outcomes: tuple[OutcomeSpec, ...]`, `capability`), and the fingerprint is `CommandContract.fingerprint()`. Validation therefore consumes a normalized `ContractInfo` (name, arguments, result schema, outcomes, execution fingerprint) and `RegistryContractLookup` is the one place either shape is known.
     3. **`CapabilityPolicy.is_empty` shipped as a method, not the `@property` of §3.1.** `_policy_is_empty` tolerates both rather than pinning one.
     4. **P1's `contracts.models.RESERVED_OUTCOMES`** is `{contract_violation, unauthorized, runtime_error}` — a rule about what a *contract* may declare. §4.6's seven-name set is about *step transition keys* and is unchanged; the two are deliberately different vocabularies. A follow-up may want `unauthorized` in §4.6's set; adding it is strictly a strengthening, since `runtime_error` already covers every unmapped reserved outcome.
     5. **§6.3's `body(f)` is refined.** Read literally ("BFS from `body_entry`, stopping at `f`") the body is a plain forward closure, which swallows every target a body step has — including the loop's own failure terminal — and makes `loop_body_escapes` vacuous, because a step's target is in its own closure by construction. The body is now that closure restricted to the steps that can reach `f` again, and a body step may legally target a body step, `f` itself, or one of `f`'s **own declared exits** (`transitions` plus `continuation`) — the literal reading of "the loop's only exits are its own transitions". The golden fixture's `open-gate -> sweep-failed` edge is legal under this rule and would be an error under the vacuous one.
     6. **§6.8's 47 codes are partitioned, not flattened.** `VALIDATOR_CODES` (40) are what `validate_definition` emits; `MODEL_CODES` (`empty_boolean_operand`, `duplicate_step_id`) are rejected by the strict models and by `load_definition_json`'s duplicate-key hook before validation runs; `COMPILER_ONLY_CODES` (`authority_field_ignored`, `requires_agent_proposal`, `ambiguous_prose`, `source_ref_out_of_range`, `excerpt_truncated`) need the Markdown source, which `validate_definition` deliberately does not take, and belong to commit 3's slice. `tests/fixtures/playbooks/v2/invalid/` carries one fixture for each of the first two groups (42 files) and the suite asserts the partition is exact.
     7. **§17.2 is applied, and goes further than items 1–2 said it would.** The shared golden fixture did not satisfy the locked §4.5 model as checked in. Applied to `review-pipeline.artifact.json` *and* `review-pipeline.v6.artifact.json`: `spec.created` -> `spec.approved` with the rule renamed `sweep-on-spec-approved`; `classify-risk` gained `prompt` and `outcome_field: "risk"`; `await-approval` gained `awaited` and `outcomes: ["approve", "revise"]`; `escalate`'s `objective` became a `LiteralValue` rather than a bare string. Three further changes were needed for the artifact to validate against the **real** registry rather than merely to parse: the command steps now map the contracts' actual outcomes (`created`/`reused`/`rejected` for `ensure_task`, `listed` for `list_tasks`, `created`/`reused`/`skipped`/`rejected` for `gate_create`) instead of the invented `success`/`failure`; `ensure-review-task` supplies the required `dedup_key`; the `task.completed` trigger filter reads the declared `review_task` field instead of the undeclared `task_type`; and `classify-risk`, `escalate`, `await-approval` and `for-each-task` each gained the `runtime_error` catch-all §4.6 requires. `review-pipeline.receipts.json` and the three coupled assertions in Package 5's `tests/test_playbook_v2_fixtures.py` were updated to match.
     8. **§9.1's suite split is collapsed.** With P1 landed there is nothing to skip: the golden artifact validates against the real `CONTRACTS` and the real event registry with **zero** errors and zero warnings, its only finding being the `delegation_runtime_checked` info on `escalate`. `TestGoldenArtifactContracts` now asserts the recorded `compiled_against` fingerprints still match the live registry — a drift there is precisely what `stale_contract` means in production.
     9. **`jsonschema` is declared in commit 2, not commit 4** (§8, §20 item 7). `validation.py` calls `Draft202012Validator.check_schema`, so it is this commit's runtime need; deferring it would have meant an import guard and a silent skip, which §20 item 7 explicitly rules out.
     10. **`IdentifierInventory` is a `Protocol` in `validation.py`**, not an import of `authoring.py`. Validation runs on an artifact, with or without the Markdown it came from, and commit 3 owns `authoring.py`; the frozen dataclass there satisfies the protocol structurally.
     11. **A test helper module was added** that the roadmap's file list does not name: `tests/playbook_v2_helpers.py` holds the stub lookups, the valid twin, and the one-defect mutation table behind `invalid/`. Keeping the generator beside the suite is what lets `test_the_checked_in_fixtures_match_their_generator` turn a fixture edit into a reviewable diff instead of a silent divergence.
