...
| §5.5 T-13 assertion 4 reads "a stale artifact belonging to a disabled **or acknowledged** playbook does not fail the check" | `playbook_migration_acknowledge` writes one row in `playbook_migration_acks` and never touches `playbook_activations`, so a playbook could be acknowledged *and* enabled at the same time. `release_check` skipped every row carrying `acknowledged_by`, which suppressed the compatibility check for execution that was really happening | **An acknowledgement no longer excuses an enabled activation** (`sound-horizon-20`). Only `enabled: false` skips a row; the waiver table is no longer joined into the daemon's release-check rows at all. The escape hatch survives for the state the waiver actually describes — an operator who means it disables the activation, and the disabled row is then skipped as before. The inventory's §3.3 rule is unchanged: an acknowledged entry is still `disabled` there, because that disposition is about V1→V2 readiness, not about what the daemon is executing |
...
7. `test_every_command_resolves` — every command the artifact invokes is registered in the Package 1 contract registry, and the artifact's `compiled_against.commands[name]` equals that contract's current fingerprint. `test_compiled_against_fingerprints_every_referenced_profile` is the profile half of the same assertion: `compiled_against.profiles` names exactly the profiles the artifact depends on — including a *delegated* one, passed to a command as a literal `profile_id` argument, which is the only kind `default-pipeline` has — and each fingerprint equals the shipped profile's current capability fingerprint.
...
4. `test_disabled_playbooks_do_not_block` — a stale artifact belonging to a disabled playbook does not fail the check, acknowledged or not. `test_an_acknowledged_but_enabled_activation_still_blocks` locks the converse: a waiver does not excuse a row that is still enabled (`sound-horizon-20`).
