...
### 16.11 Exhaustive step explanations on 2026-09-03 (task agile-cascade-53)

Package 5's exit gate (solid-harbor.49) found that the live projection did not
compose with the rich fixture the dashboard slice was written against.
`src/playbooks/graph_projection.py` explained the `command` family from its
contract and gave the other six families one shared clause with `inputs=[]`
and `result=None`, so the projected card hid every `llm` prompt, `agent_task`
objective, `wait` awaited/correlation key, `foreach` collection, decision case
condition, terminal result and non-command `save_result_as` binding. §16.10
deviation 1 could not have caught it: T-44 compared `[id, step_kind]` and
`[edge id, kind]` pairs only.

What changed:

1. **One typed explanation builder per family** (`_llm_explanation`,
   `_agent_task_explanation`, `_decision_explanation`, `_wait_explanation`,
   `_foreach_explanation`, `_terminal_explanation`). Each projects the values
   the step reads as `inputs`, the binding it writes as `result` (typed by
   `result_schema_for`), and typed `EffectClauseDTO` rows for what it does.
   `renderer` stays `"canonical"` — there is no contract presentation for
   these families — which §4.2 already defines as "show every executable
   field", not "show nothing".
2. **Conditions render as text.** `_condition_text` walked to readable form
   (`gate.created == false`) for both the decision card's input rows and
   `GraphEdgeDTO.condition`, which previously carried canonical JSON. The
   canonical form is unchanged in `advanced.typed_step` and in the row's
   `value.canonical`.
3. **`project_value` covers the whole §4.1 grammar** — `context_ref`,
   `coalesce`, `list` and `object` had fallen through to a JSON dump.
4. **Per-family badges**: LLM budget and tool count, agent-task wait policy,
   wait kind, loop failure policy.
5. **T-44 is now whole-response equality.**
   `dashboard/src/pages/playbook-graph-v2/__tests__/graph.fixture.json` is
   generated by `python -m tests.playbook_v2_helpers`
   (`project_golden_graph` / `write_graph_fixture`) and the test asserts the
   file equals the projection byte for byte, plus a second test pinning the
   semantic fields themselves so a regeneration that silently emptied them
   cannot stay green.

Still open, and filed rather than folded in: `__tests__/fixtures.ts` is still
