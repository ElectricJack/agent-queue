...
| The supervisor is refused `provider_held_tasks` or another `provider_*` command | `aq doctor --check profiles.system_drift` | The vault copy of the supervisor profile predates the grant (seeding never overwrites it). `aq agent profile-reseed --profile-id supervisor --grants-only` merges just the missing grants and keeps every other edit. |
