...
lock for their lifetime. At pool startup, a worker removes only matching clones
whose owner lock has gone away, so a SIGTERM'd pool is reclaimed on the next
test run. The sweep tries at most eight databases per startup, with a five
second timeout per drop; failures warn and remain for a later run. Older pool
names, worker databases, and scratch databases have no such lock and still
require manual inspection before removal. If an explicit/reused
