...
| [`src/database/queries/integration_train_queries.py`](../../../src/database/queries/integration_train_queries.py) | The projections used to seal an integration train atomically. | [reference/database/tables.md](../database/tables.md) | `tests/test_integration_sealing.py` |
...
| [`src/database/queries/provider_availability_queries.py`](../../../src/database/queries/provider_availability_queries.py) | `provider_availability` rows, the append-only `provider_availability_transitions` log, and the once-per-generation notification claim. | [reference/database/tables.md](../database/tables.md) | The availability service is the only writer and serialises per provider, so an upsert needs no compare-and-swap. `tests/test_provider_availability.py` |
...
| [`src/database/queries/task_reroute_queries.py`](../../../src/database/queries/task_reroute_queries.py) | Applies and undoes a re-route (`tasks.profile_id`, `tasks.rerouted_from` and a `task_reroutes` row in one transaction) and serves the trickle count and per-task move statistics. | [reference/database/tables.md](../database/tables.md) | Guarded by the same "no worker holds it" predicate as `update_task_routing`, so a claim that wins after the planner's read is never retargeted. `tests/test_provider_reroute.py` |
| [`src/database/queries/transaction_queries.py`](../../../src/database/queries/transaction_queries.py) | `immediate()` — the write-transaction context every caller-owned query method composes inside. | [reference/database/queries.md](../database/queries.md) | On PostgreSQL this is `engine.begin()`; the name is a leftover from the SQLite era's `BEGIN IMMEDIATE`. |
