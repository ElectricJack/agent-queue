...
| [`src/integration/delegate_release.py`](../../../src/integration/delegate_release.py) | Settles the verifier, repair-stage and candidate-member tickets of an operation that has ended, and answers whether a *live* operation still owns a task so a removal can be refused with a reason. | [guides/integration-troubleshooting.md](../../guides/integration-troubleshooting.md) | Any mode. One implementation shared by the reconciliation tick, `integration_abort`, `cancel_preserving` and `aq doctor --check integration.stranded_delegates --fix`. Settles the ticket only; a retained owner or workspace lock is named, never released. `tests/test_integration_delegate_release.py` |
...
| [`src/integration/child_delivery.py`](../../../src/integration/child_delivery.py) | Proves a COMPLETED child's published head from Git and records the approved `leaf` evidence its collecting parent needs to assemble it (`ChildDelivery.ensure_evidence`, called by `CollectionService`); diagnoses/re-drives one stuck child (`aq integration redrive-child`) and backs `aq doctor --check integration.stuck_children`. | [guides/integration-troubleshooting.md](../../guides/integration-troubleshooting.md) | Hierarchy and train. Never overrides a reviewer's rejection or an open reviewer task; a failed proof backs off per child and head. `tests/test_integration_child_delivery.py` |
| [`src/integration/root_pull_requests.py`](../../../src/integration/root_pull_requests.py) | Opens the pull request a COMPLETED train root still lacks (`RootPullRequestReconciler`, ticked by `IntegrationService`), and diagnoses/re-drives one stuck root (`RootDeliveryRedrive`, `aq integration redrive-root`). | [guides/integration-troubleshooting.md](../../guides/integration-troubleshooting.md) | Train. Creates PRs through `EpicPullRequestService`; skips unverified epics, delivered roots and heads already on the default branch. `tests/test_integration_root_pull_requests.py` |
| [`src/integration/identity_rebind.py`](../../../src/integration/identity_rebind.py) | Proves and retires a branch origin and checkpoint that a live task inherited from a deleted task with the same id (`aq integration rebind-reused-identity`): every predecessor commit and the ref tip must be on the default branch, or the ref gone, or the operator names the exact commit with `--discard-tip`. | [specs/design/integration-identity-diagnostics.md](../../specs/design/integration-identity-diagnostics.md) | Development (and disabled/observe); refuses hierarchy/train projects, live writers, a held owner and dependent integration history. Dry run by default; applying re-proves under the hierarchy lock, keeps the retired origin, copies the checkpoint into an `integration.task_identity_rebound` event and touches no Git ref or owner row. `tests/test_integration_identity_rebind.py` |
...
| [`src/integration/stale_schedule.py`](../../../src/integration/stale_schedule.py) | Classifies a train project's outstanding sweep request (`none`, `active`, `in_flight`, `unsealed`, `blocked`, `stale`) and releases one nothing else will end — its batch aborted, empty, gone, or promoted without the lease release consumes — with the fenced lease and an `integration.schedule_request_released` event. | [guides/integration-troubleshooting.md](../../guides/integration-troubleshooting.md) | Train. Read by `IntegrationScheduler.mark_due` (before the settling gate), `integration_release`, `integration_abort`, `cancel_preserving`, `aq integration clear-stale-request` and `aq doctor --check integration.stale_schedule`. Never touches Git or a batch row; unresolved write evidence keeps a request `blocked`. `tests/test_integration_stale_schedule.py` |
