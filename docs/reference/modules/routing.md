...
| [`src/profiles/defaults/`](../../../src/profiles/defaults/) — generic workers | 2 — `worker-claude`, `worker-codex` | The shipped generic workers: one per harness, `lifecycle: task`, with `default_class` only as the fallback for a task that names no class. | [concepts/agents-and-routing.md](../../concepts/agents-and-routing.md) |
