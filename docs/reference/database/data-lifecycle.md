...
* durable integration bookkeeping still names a task in the subtree —
  ...
  `integration_candidate_resolutions.repair_task_id`. A held descendant pins
  its root, since the subtree moves together. The refusal lands before anything
  is written, rather than as a constraint error from the final `DELETE`.
  [`task_references.py`](../../../src/database/queries/task_references.py)
  declares what happens to every foreign key onto `tasks`, and
  `tests/test_hierarchy_archive_delete.py` fails when a new one is undeclared.
...
per task and carry on. They also catch any other exception from a single root,
such as an `IntegrityError` from a foreign key nothing declares yet, and log it
at `warning` as the exception class plus the constraint that refused. Before
that, a single held task raised a `RestrictViolationError` out of the loop and
the sweep archived *nothing* — every unrelated eligible task included — and the
orchestrator's rate limit then kept it from trying again for an hour, at which
point it met the same row. The hourly sweep also records why it skipped each
root (`task_metadata` key `archive_refusal`, code `unexpected` for the
exception case); `aq doctor --check tasks.archive_blocked` and
`aq task archive-settings` report it — see
...
| `integration_owned` | An active repair operation owns a task in the subtree, or durable integration bookkeeping still names one: `integration_parent_episodes.parent_task_id`, `integration_parent_verifications.parent_task_id`, `integration_repair_operations.verifier_task_id` or `integration_candidate_resolutions.repair_task_id`, each a **named `RESTRICT`** foreign key. | Let an active operation finish or be cancelled. The bookkeeping rows are append-only audit and cannot be removed, so a task they name stays; the control plane's identity may not dangle. |
...
* The RESTRICT-protected integration tables are deliberately *not* cleaned up
  ...
  points at nothing. Delete and archive refuse those up front with
  `integration_owned`.
