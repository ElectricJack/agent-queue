...
| `task_reroutes` | Append-only history of provider re-routes: every automatic move, operator-forced move and undo. `tasks.rerouted_from` is its current projection. | `task_id`, `from_profile_id`, `to_profile_id`, `from_provider`, `to_provider`, `reason_code`, `batch_id`, `undone_at` | check `reason_code IN ('provider_unavailable','operator_forced','operator_undo')`; the move and its row are written in one transaction under the "no worker holds it" routing guard |
...
| `provider_availability` | One row per provider key (a harness login, plus `llm` for the direct path): its derived state, reason, expected recovery, evidence ring and any operator override. Written only by the daemon's availability service. | `state`, `reason_code`, `until`, `level`, `evidence`, `override_state`, `override_until`, `generation`, `probation_from` | checks on `state` (the six values; the derived state never holds `disabled`) and `override_state` (`disabled`/`available`); the effective state is computed from both |
| `provider_availability_transitions` | Append-only log of every change of a provider's effective state — what `aq provider history` and the digest's provider facts read. | `provider`, `from_state`, `to_state`, `reason_code`, `until`, `generation`, `actor`, `at` | append-only |
