...
- Emergent work: file it with `aq task create --reason "..."`; the server writes the `discovered-from` edge back to the task you hold. If your task is a child of an epic, the new task is placed as your sibling under that same epic by default (no `--parent` needed); pass `--parent <your-task-id>` only to nest it under your own task, or `--root` for cross-cutting work that belongs at project level and must go through routing. `--parent` and `--root` are mutually exclusive. Only your task, its descendants and its immediate parent are selectable as parents. A finding you nested under your own task blocks your close (`hierarchy.open_children`); move it aside with `aq task reparent --task-id <id> --parent-id <container-id>` (or `--root`) instead of re-filing it — a worker may reparent only unclaimed tasks it filed, to the same parents it could file under.
...
- Before a passing close, re-read the plan section you implemented and reconcile the task's Deliverables checklist. Record each test command with `--test` and other checks (lint, build) with `--command`; a test or command item declared as a command line is met by a matching recorded value; if an item is deliberately unshipped, use `--deliverable-unmet 'id: reason'` so the reviewer sees the exception.
