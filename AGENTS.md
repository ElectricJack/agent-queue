...
- Emergent work: file it with `aq task create --reason "..."`; the server writes provenance back to the task you hold. New work defaults to a child of the task you hold, so it remains visible and blocks that task's successful completion while outstanding. Pass `--parent <id>` only to deliberately choose an authorized alternative parent, or `--root` for cross-cutting root work; `--parent` and `--root` are mutually exclusive. Only your task, its descendants and its immediate parent are selectable as parents. Do not move a child aside merely to close its source task: complete or otherwise resolve the child first.
- Close explicitly: push your branch (open a PR only when the task or project requires one), then `aq task close <id> --outcome pass --summary "..."`. Heartbeat (`aq task heartbeat <id>`) before anything that runs quiet for minutes.
- Before a passing close, re-read the plan section you implemented and reconcile the task's Deliverables checklist. Record each test command with `--test` and other checks (lint, build) with `--command`; a test or command item declared as a command line is met by a matching recorded value; if an item is deliberately unshipped, use `--deliverable-unmet 'id: reason'` so the exception stays visible.
