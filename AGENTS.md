...
- Emergent work: file it with `aq task create --reason "..."`; the server writes the `discovered-from` edge back to the task you hold. If your task is a child of an epic, the new task is placed as your sibling under that same epic by default (no `--parent` needed); pass `--parent <your-task-id>` only to nest it under your own task. Only your task, its descendants and its immediate parent are selectable; work that does not belong to the epic at all still lands as your sibling — say so in its description so the epic owner can reparent it.
