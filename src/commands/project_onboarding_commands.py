...
contract's request validation and scope policy. GitHub discovery delegates to
``GhClient``; repository mutations and durable status reads delegate to
``ProjectOnboardingService``. Root discovery remains a structured stub for its
owning package.
...
class ProjectOnboardingCommandsMixin:
    ...
    # packages replace the remaining root stubs without changing the
