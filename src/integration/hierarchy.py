...
class HierarchyIntegration:
    ...
    async def reconcile_unmaterialized_tasks_on(
        ...
        *,
        ...
        """
        if task_ids is None:
            task_ids = await self.preflight_reconcile_unmaterialized_tasks_on(
                conn,
    ...
    async def preflight_reconcile_unmaterialized_tasks_on(
        self,
        conn,
        *,
        project_id: str,
        ...
        """Prove that a legacy graph is pristine before changing its generation."""
        repo = await self._repo_on(conn, repository_id)
        if repo is None or repo.project_id != project_id:
            raise HierarchyError("invalid", "designated repository is not in the project")
        ...
        if origins:
            raise HierarchyError(
                "invalid", "task already has branch origin history and is not an unmaterialized rollout task"
            )
        checkpoints = list(
            (
                await conn.execute(
