...
async def run(config_path: str, profile: str | None = None) -> bool:
    ...
    async def run_scheduler():
        ...
        if discord_transport is not None and config.discord.channel_id:
            ...
            # Every Discord link names the dashboard server's configured
            # public origin (src/remote_links.py), never the daemon's health
            # port: the daemon serves no dashboard pages.
            dashboard_link = await orch.dashboard_link.resolve()
            if not dashboard_link.url:
                logger.warning(
                    "Discord posts will carry a notice instead of a dashboard link (%s): %s. "
                    "`aq dashboard link` explains the setting.",
                    dashboard_link.reason,
                    dashboard_link.detail,
                )
            orch.escalation_delivery = EscalationDeliveryService(
                ...
                link_resolver=orch.dashboard_link,
