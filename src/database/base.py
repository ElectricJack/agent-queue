...
class DatabaseBackend(Protocol):
    ...
    # --- Hierarchical integration durable state ---
    ...
    async def reserve_integration_conflict_resolution(
        self, conn, intent_id: str, values: dict
    ) -> dict: ...
    ...
    # --- Durable supervisor-owned human escalations ---
    async def create_escalation(self, **values) -> tuple[dict, bool]: ...
    ...
    async def transition_escalation(
        self, escalation_id: str, *, expected_revision: int, new_state: str, **values
    ) -> dict | None: ...
    async def accept_escalation_reply(self, escalation_id: str, **values) -> dict: ...
    async def resolve_legacy_accepted_escalation(
        self, escalation_id: str, **values
    ) -> dict | None: ...
    async def resolve_escalation_on_recovery(self, escalation_id: str, **values) -> dict | None: ...
    async def get_escalation_message(self, reply_id: str) -> dict | None: ...
    async def append_escalation_message(
        self, escalation_id: str, **values
    ) -> tuple[dict, bool]: ...
    async def list_escalation_messages(self, escalation_id: str) -> list[dict]: ...
    async def find_escalation_by_thread(
        self, *, channel_id: str, thread_id: str
    ) -> dict | None: ...
    async def begin_escalation_action(self, escalation_id: str, **values) -> dict: ...
    async def finish_escalation_action(self, action_id: str, **values) -> dict: ...
    async def list_escalation_actions(self, escalation_id: str) -> list[dict]: ...
    ...
    async def claim_escalation_deliveries(self, **values) -> list[dict]: ...
    async def finish_escalation_delivery(self, delivery_id: str, **values) -> dict | None: ...
    async def get_escalation_delivery(self, delivery_id: str) -> dict | None: ...
    async def list_escalation_deliveries(
        self, escalation_id: str, *, statuses=None
    ) -> list[dict]: ...
    async def reserve_digest_window(self, **values) -> tuple[dict, bool]: ...
    async def complete_digest_evaluation(self, window_id: str, **values) -> dict | None: ...
    async def claim_digest_windows(self, **values) -> list[dict]: ...
    async def finish_digest_delivery(self, window_id: str, **values) -> dict | None: ...
    async def get_digest_window(self, window_id: str) -> dict | None: ...
    async def get_report_request(self, request_id: str) -> dict | None: ...
    async def request_report(self, request_id: str, *, now: float) -> dict | None: ...
    async def submit_hourly_report(self, request_id: str, **values) -> dict | None: ...
    async def cancel_hourly_reports(self, *, now: float) -> int: ...
    async def invalidate_hourly_visibility(self, **values) -> int: ...
    async def list_digest_windows(
        self, *, destination=None, config_generation=None, statuses=None, limit: int = 100
    ) -> list[dict]: ...
    async def count_due_escalation_deliveries(self, now: float) -> int: ...

    # -- provider quota snapshots -----------------------------------------
    async def record_provider_usage(self, snapshots) -> int: ...
    async def latest_provider_usage(self, provider=None) -> list[dict]: ...
    async def provider_usage_series(
        self, provider, window, scope="", *, since=0.0, limit=10_000
    ) -> list[dict]: ...
    async def purge_provider_usage(self, before, *, limit=1000) -> int: ...

    # -- provider availability (provider-failover D7) ----------------------
    async def list_provider_availability(self) -> list[dict]: ...
    async def get_provider_availability(self, provider: str) -> dict | None: ...
    async def save_provider_availability(self, row, transition=None) -> None: ...
    async def list_provider_transitions(
        self, provider=None, *, since=None, limit: int = 50
    ) -> list[dict]: ...
