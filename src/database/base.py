...
class DatabaseBackend(Protocol):
    ...
    # Durable agent waits (registration and completion share producer transactions).
    ...
    async def list_morning_reports(self, **values) -> list[dict]: ...
    async def latest_morning_reservation(self) -> dict | None: ...
    async def reserve_morning_report(self, **values) -> tuple[dict | None, str]: ...
    async def claim_morning_build(self, report_id: str, **values) -> dict | None: ...
    async def store_morning_build(self, report_id: str, **values) -> dict | None: ...
    async def fail_morning_build(self, report_id: str, **values) -> None: ...
    async def finalize_morning_fallback(self, report_id: str, **values) -> dict | None: ...
    async def cancel_pending_morning_reports(self, **values) -> int: ...
    async def prune_morning_reports(self, **values) -> int: ...
    async def reserve_outbound_delivery(self, **values) -> tuple[dict, bool]: ...
    async def get_outbound_delivery(self, delivery_id: str) -> dict | None: ...
    async def list_outbound_deliveries(self, **values) -> list[dict]: ...
    async def claim_report_deliveries(self, **values) -> list[dict]: ...
    async def finish_outbound_delivery(self, delivery_id: str, **values) -> dict | None: ...
    ...
    # --- Operator conversations with the global supervisor ---
    async def accept_conversation_input(self, **values) -> dict: ...
    ...
    async def get_conversation_input(self, input_id: str) -> dict | None: ...
    async def count_conversation_inputs(
        self, *, since: float, author_id: str | None = None, channel_id: str | None = None
    ) -> int: ...
    async def bind_conversation_thread(
        self, conversation_id: str, *, external_thread_id: str, now: float
    ) -> bool: ...
    async def set_conversation_state(
        self, conversation_id: str, *, state: str, now: float, expected: tuple[str, ...] = ()
    ) -> bool: ...
    ...
    async def get_backfill_cursor(self, *, transport: str, channel_id: str) -> dict | None: ...
    async def list_backfill_cursors(self) -> list[dict]: ...
    async def advance_backfill_cursor(
        self, *, transport: str, channel_id: str, last_external_message_id: str, now: float
    ) -> None: ...
    async def record_intake_gap(self, **values) -> dict: ...
    async def list_intake_gaps(self, *, limit: int = 50) -> list[dict]: ...
    async def conversation_counts(self) -> dict: ...
    ...
    # -- immutable test selection history --------------------------------
