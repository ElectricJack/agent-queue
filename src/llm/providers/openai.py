"""OpenAI-compatible chat provider (OpenAI API or a local/self-hosted endpoint).

Ollama and other local servers expose an OpenAI-compatible API, so this
provider uses the ``openai`` Python SDK for both the real OpenAI API and
local endpoints. Format conversion between the internal Anthropic-style
types and OpenAI format is handled by the shared ``openai_adapter`` module.
"""
...
class OpenAIProvider(LLMProvider):
    """Chat provider using the OpenAI API or an OpenAI-compatible local endpoint."""
