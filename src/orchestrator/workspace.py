...
class WorkspaceMixin:
    ...
    async def aconfirm_integration_owner_handoff(self, owner: dict) -> bool:
        ...
        # Public transfer must stop a live writer. The no-stop pool helper is
        # reserved for that worker's own close, never an external transfer.
    ...
    async def arecover_completed_integration_pool_claim(self, task, session) -> bool:
        ...
        if session.desired_state == "running":
            stopped = await self.db.update_session_instance(
                session.id,
        ...
        release = await self.db.release_claim(
            ...
            # On this branch ``release_claim`` preserves the requested
            # terminal state when it matches ``expected_task_status``.  The
            # session was fenced to ``stopped`` immediately before the
            # ownership handoff, so draining is a harmless idempotent guard.
