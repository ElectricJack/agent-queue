...
- **Output contract**: `--json` prints the versioned envelope from `envelope.py`
  ...
  the pre-envelope raw-payload shape for one release. Both are options on the top-level `aq`
  group, so they go before the subcommand. New commands should route their output through
  `emit()` rather than printing JSON directly.
