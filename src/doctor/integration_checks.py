...
async def _find_stranded_fences(ctx: DoctorContext) -> list[dict]:
    ...
    A row qualifies only when there is provably no writer: no live session for
    the owning task, no workspace still locked by it, and the task itself is
    not ASSIGNED/IN_PROGRESS (or is gone entirely).  ``collector`` rows are
    excluded -- their owner is an operation, not a task, so none of those
    questions are even askable of them.
    """
    from sqlalchemy import select
...
async def _check_stranded_fences(ctx: DoctorContext) -> CheckResult:
    ...
    return CheckResult(
        ...
        severity=Severity.WARN,
        detail=(
            f"{len(stranded)} integration branch(es) are held '{first['handoff_state']}' by a "
            ...
            "'canonical branch is not reserved by this task'. Return them to their own "
            "reserved fence with `aq doctor --check integration.stranded_fences --fix`"
        ),
        fixable=True,
...
async def _fix_stranded_fences(ctx: DoctorContext) -> CheckResult:
    """Return each dead row to a fresh ``reserved`` fence for its own owner.

    This is a self-transfer, not a handoff: the owner and the role do not
    change, so no second writer is granted anything.  The fence token is bumped
    anyway, so that if a writer somehow *were* still alive against the old
    token its next fenced write loses rather than racing.  The CAS re-checks
    the exact row and state that was diagnosed, so a row that came back to life
    between the check and the fix is left alone.
    """
    from sqlalchemy import update
    ...
    async with ctx.db.immediate() as conn:
        for row in stranded:
            result = await conn.execute(
                ...
                .where(
                    integration_branch_owners.c.id == row["owner_row_id"],
                    integration_branch_owners.c.owner_id == row["task_id"],
                    integration_branch_owners.c.owner_role == row["owner_role"],
                    integration_branch_owners.c.fence_token == row["fence_token"],
                    integration_branch_owners.c.handoff_state == row["handoff_state"],
                )
                .values(
    ...
    return CheckResult(
        id="integration.stranded_fences",
        severity=Severity.OK if len(repaired) == len(stranded) else Severity.WARN,
        detail=(
            f"returned {len(repaired)} of {len(stranded)} stranded fence(s) to a fresh "
            "reserved reservation for their own owner"
        ),
        fixable=True,
        fix_applied=True,
        data={"count": len(repaired), "fences": repaired},
    )
...
def integration_checks() -> list[DoctorCheck]:
    return [
        ...
        # Fixable because the repair is a self-transfer with no successor: the
        # row goes back to ``reserved`` for the owner it already names, and
        # only after the check has established that no session, no workspace
        # lock and no running task still refer to it.
        DoctorCheck(
            id="integration.stranded_fences",
