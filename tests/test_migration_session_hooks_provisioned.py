...
def _numeric_boolean_defaults(path: Path) -> list[str]:
    """Return ``file:line`` for Boolean Columns with numeric defaults.

    Supports migration declarations (``sa.Column(..., sa.Boolean(), ...)``)
    and metadata declarations (``Column(..., Boolean, ...)``).
    """
...
def test_tables_metadata_has_no_numeric_boolean_defaults():
    """Autogenerate uses ``tables.py`` defaults when producing migration DDL."""
