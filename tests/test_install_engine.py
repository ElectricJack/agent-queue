...
def test_the_built_in_registry_admits_the_host_before_it_checks_anything_else():
    ...
    # Adapters chain onto each other rather than all naming the host step, so
    # the invariant is that every step depends on it *transitively*.
    reachable = {STEP_HOST}
    for step in registry.ordered()[1:]:
        assert reachable.intersection(step.depends_on), f"{step.id} floats free of {STEP_HOST}"
