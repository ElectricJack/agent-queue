"""Project onboarding saga tests (design §3.1, §6, §7)."""

from __future__ import annotations

import asyncio
import hashlib
import logging
import os
from pathlib import Path
import subprocess

import pytest

from src.commands.contracts.project_onboarding import (
    ProjectOnboardingError,
    parse_onboard_project_request,
)
from src.commands.handler import CommandHandler
from src.config import AppConfig, ProjectRoot
from src.database import Database
from src.git.manager import GitError, GitManager
from src.models import Project, RepoSourceType, Workspace
from src.orchestrator import Orchestrator
from src.projects.github import GhClient
from src.projects.onboarding import ProjectOnboardingService
from tests.pg_dsn import ensure_worker_postgres_dsn


FAKE_GH = Path(__file__).parent / "fixtures" / "fake_gh" / "gh"
POSTGRES_TEST_DSN = ensure_worker_postgres_dsn()
CREDENTIALED_GIT_ERROR = (
    "fatal: unable to access "
    "'https://alice:ghp_ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789ab@github.com/acme/widgets.git': "
    "GH_TOKEN=ghp_ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789ab rejected"
)


def _git(path: Path, *args: str, check: bool = True) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        ["git", "-C", str(path), *args],
        check=check,
        capture_output=True,
        text=True,
    )


def _make_repo(path: Path, *, remote: str | None = None) -> Path:
    path.mkdir(parents=True)
    subprocess.run(
        ["git", "init", "--initial-branch", "main", str(path)],
        check=True,
        capture_output=True,
        text=True,
    )
    (path / "tracked.txt").write_text("untouched\n", encoding="utf-8")
    _git(path, "add", "tracked.txt")
    _git(
        path,
        "-c",
        "user.name=Onboarding Test",
        "-c",
        "user.email=onboarding@example.invalid",
        "commit",
        "-m",
        "seed",
    )
    if remote:
        _git(path, "remote", "add", "origin", remote)
    return path


def _make_bare_remote(
    path: Path,
    *,
    branch: str = "trunk",
    with_commit: bool = True,
    owner_entry: str | Path | None = None,
) -> Path:
    worktree = path.parent / f"{path.name}-worktree"
    worktree.mkdir(parents=True)
    subprocess.run(
        ["git", "init", "--initial-branch", branch, str(worktree)],
        check=True,
        capture_output=True,
        text=True,
    )
    if with_commit:
        (worktree / "from-remote.txt").write_text("cloned\n", encoding="utf-8")
        if isinstance(owner_entry, Path):
            (worktree / ".aq-onboarding-owner").symlink_to(owner_entry)
        elif owner_entry is not None:
            (worktree / ".aq-onboarding-owner").write_text(owner_entry, encoding="utf-8")
        _git(worktree, "add", "--all")
        _git(
            worktree,
            "-c",
            "user.name=Onboarding Test",
            "-c",
            "user.email=onboarding@example.invalid",
            "commit",
            "-m",
            "remote seed",
        )
    subprocess.run(
        ["git", "clone", "--bare", str(worktree), str(path)],
        check=True,
        capture_output=True,
        text=True,
    )
    return path


class _LocalRemoteGitManager(GitManager):
    """Run real Git while substituting one local bare repo for GitHub I/O."""

    def __init__(self, remote: Path) -> None:
        super().__init__()
        self.remote = remote

    async def acreate_checkout(self, repo_url: str, checkout_path: str) -> None:
        await super().acreate_checkout(str(self.remote), checkout_path)

    async def _arun(self, args, cwd=None, timeout=None):
        if args and args[0] == "push" and "origin" in args:
            args = [str(self.remote) if arg == "origin" else arg for arg in args]
        return await super()._arun(args, cwd=cwd, timeout=timeout)


def _git_with_local_remote(remote: Path) -> GitManager:
    return _LocalRemoteGitManager(remote)


def _fake_gh(tmp_path: Path, *, fail_stderr: str | None = None) -> tuple[GhClient, Path]:
    argv_log = tmp_path / "gh-argv.log"
    env = {**os.environ, "FAKE_GH_ARGV_LOG": str(argv_log)}
    if fail_stderr is not None:
        env["FAKE_GH_FAIL_STDERR"] = fail_stderr
    return (
        GhClient(
            executable=str(FAKE_GH),
            env=env,
        ),
        argv_log,
    )


def _gh_invocations(argv_log: Path) -> list[list[str]]:
    if not argv_log.exists():
        return []
    calls = []
    for record in argv_log.read_bytes().split(b"\x1e"):
        if record:
            calls.append([part.decode() for part in record.split(b"\0")[:-1]])
    return calls


def _tree_digest(path: Path) -> str:
    digest = hashlib.sha256()
    for item in sorted(path.rglob("*"), key=lambda p: p.as_posix()):
        digest.update(item.relative_to(path).as_posix().encode())
        if item.is_file():
            digest.update(item.read_bytes())
    return digest.hexdigest()


def _repository_snapshot(path: Path) -> tuple[str, str, str, str, str]:
    """Capture stable worktree and Git semantics, excluding volatile .git internals."""
    digest = hashlib.sha256()
    for item in sorted(path.rglob("*"), key=lambda p: p.as_posix()):
        relative = item.relative_to(path)
        if relative.parts[0] == ".git":
            continue
        digest.update(relative.as_posix().encode())
        if item.is_file():
            digest.update(item.read_bytes())
    return (
        digest.hexdigest(),
        _git(path, "status", "--porcelain=v1", "--untracked-files=all").stdout,
        _git(path, "for-each-ref", "--format=%(refname) %(objectname)").stdout,
        _git(path, "symbolic-ref", "-q", "HEAD", check=False).stdout,
        _git(path, "config", "--local", "--list").stdout,
    )


def _request(**overrides):
    values = {
        "request_id": "request-1",
        "source_mode": "link",
        "root_id": "dev",
        "relative_path": "repo",
        "project_name": "Example Project",
        "project_id": "example-project",
        "default_branch": None,
    }
    values.update(overrides)
    return parse_onboard_project_request(values)


@pytest.fixture(params=["sqlite", "postgres"])
async def onboarding(request, tmp_path: Path):
    root = tmp_path / "root"
    root.mkdir()
    data_dir = tmp_path / "data"
    config = AppConfig(
        project_roots=[ProjectRoot(id="dev", label="Development", path=str(root))],
        data_dir=str(data_dir),
        database_path=str(tmp_path / "onboarding.db"),
    )
    if request.param == "postgres":
        if not POSTGRES_TEST_DSN:
            pytest.skip("POSTGRES_TEST_DSN not set")
        from src.database.adapters.postgresql import PostgreSQLDatabaseAdapter

        database = PostgreSQLDatabaseAdapter(POSTGRES_TEST_DSN)
        await database.initialize()
        await database.reset_for_tests()
    else:
        database = Database(config.database_path)
        await database.initialize()
    service = ProjectOnboardingService(database, config, GitManager())
    yield service, database, config, root, data_dir
    await database.close()


@pytest.mark.parametrize(
    ("remote", "expected_remote"),
    [
        (None, None),
        ("https://example.invalid/acme/repo.git", "https://example.invalid/acme/repo.git"),
        ("https://token:secret@example.invalid/acme/repo.git", "https://example.invalid/acme/repo.git"),
    ],
)
async def test_link_registers_repository_without_modifying_it(
    onboarding, remote, expected_remote
):
    service, database, _, root, data_dir = onboarding
    repo = _make_repo(root / "repo", remote=remote)
    before = _repository_snapshot(repo)

    result = await service.onboard_project(_request())

    assert result.project_id == "example-project"
    assert result.source_type == "link"
    assert result.canonical_path == str(repo.resolve())
    assert result.default_branch == "main"
    assert result.remote_url == expected_remote
    assert result.actions == [
        "repository_linked",
        "project_created",
        "workspace_registered",
        "vault_initialized",
    ]
    assert _repository_snapshot(repo) == before
    project = await database.get_project("example-project")
    workspaces = await database.list_workspaces("example-project")
    assert project is not None
    assert project.repo_url == (expected_remote or "")
    assert project.repo_default_branch == "main"
    assert len(workspaces) == 1
    assert workspaces[0].id == result.workspace_id
    assert workspaces[0].kind_id == "project-repo"
    assert workspaces[0].source_type is RepoSourceType.LINK
    assert workspaces[0].enabled is True
    assert (data_dir / "tasks" / "example-project").is_dir()
    assert (data_dir / "vault" / "projects" / "example-project" / "memory").is_dir()


@pytest.mark.parametrize("create_readme", [True, False])
async def test_init_publishes_main_repository_with_optional_readme(onboarding, create_readme):
    service, database, _, root, _ = onboarding
    request = _request(
        request_id=f"init-{create_readme}",
        source_mode="init",
        relative_path="new-repo",
        create_readme=create_readme,
        create_github=False,
    )

    result = await service.onboard_project(request)

    destination = root / "new-repo"
    assert result.source_type == "init"
    assert result.default_branch == "main"
    assert _git(destination, "symbolic-ref", "HEAD").stdout.strip() == "refs/heads/main"
    if create_readme:
        assert (destination / "README.md").read_text(encoding="utf-8") == (
            "# Example Project\n"
        )
        assert _git(destination, "log", "-1", "--format=%s").stdout.strip() == (
            "Initial commit"
        )
        assert "readme_committed" in result.actions
    else:
        assert not (destination / "README.md").exists()
        assert _git(destination, "rev-parse", "HEAD", check=False).returncode != 0
        assert "readme_committed" not in result.actions
    workspaces = await database.list_workspaces("example-project")
    assert len(workspaces) == 1
    assert workspaces[0].source_type is RepoSourceType.INIT


async def test_preflight_rejects_project_destination_workspace_and_root_conflicts(onboarding):
    service, database, _, root, tmp_data = onboarding
    repo = _make_repo(root / "repo")

    await database.create_project(Project(id="example-project", name="Existing"))
    with pytest.raises(ProjectOnboardingError, match="already exists") as project_error:
        await service.onboard_project(_request())
    assert project_error.value.code == "project_id_conflict"

    existing = root / "already-there"
    existing.mkdir()
    (existing / "keep.txt").write_text("do not touch\n", encoding="utf-8")
    existing_before = _tree_digest(existing)
    with pytest.raises(ProjectOnboardingError) as destination_error:
        await service.onboard_project(
            _request(
                request_id="destination",
                source_mode="init",
                relative_path="already-there",
                project_id="destination-project",
                create_readme=False,
                create_github=False,
            )
        )
    assert destination_error.value.code == "destination_conflict"
    assert _tree_digest(existing) == existing_before

    await database.create_project(Project(id="other", name="Other"))
    await database.create_workspace(
        Workspace(
            id="other-primary",
            project_id="other",
            workspace_path=str(repo.resolve()),
            source_type=RepoSourceType.LINK,
            kind_id="project-repo",
        )
    )
    with pytest.raises(ProjectOnboardingError) as workspace_error:
        await service.onboard_project(
            _request(request_id="workspace", project_id="workspace-project")
        )
    assert workspace_error.value.code == "destination_conflict"
    assert "other" in workspace_error.value.message

    outside = tmp_data.parent / "outside"
    _make_repo(outside)
    (root / "escape").symlink_to(outside, target_is_directory=True)
    with pytest.raises(ProjectOnboardingError) as escape_error:
        await service.onboard_project(
            _request(
                request_id="escape",
                relative_path="escape",
                project_id="escape-project",
            )
        )
    assert escape_error.value.code == "root_escape"


async def test_terminal_replay_and_conflicting_fingerprint(onboarding):
    service, database, _, root, _ = onboarding
    _make_repo(root / "repo")
    request = _request()

    first = await service.onboard_project(request)
    replay = await ProjectOnboardingService(database, service.config, GitManager()).onboard_project(
        request
    )

    assert replay == first
    assert len(await database.list_projects()) == 1
    assert len(await database.list_workspaces()) == 1
    with pytest.raises(ProjectOnboardingError) as conflict:
        await service.onboard_project(
            _request(project_name="Different Name", project_id="different-name")
        )
    assert conflict.value.code == "request_conflict"


class _PausingGitManager(GitManager):
    def __init__(self) -> None:
        super().__init__()
        self.init_started = asyncio.Event()
        self.continue_init = asyncio.Event()

    async def _arun(self, args, cwd=None, timeout=None):
        if args and args[0] == "init":
            self.init_started.set()
            await self.continue_init.wait()
        return await super()._arun(args, cwd=cwd, timeout=timeout)


async def test_concurrent_requests_for_same_destination_return_destination_locked(onboarding):
    _, database, config, root, _ = onboarding
    git = _PausingGitManager()
    first_service = ProjectOnboardingService(database, config, git)
    second_service = ProjectOnboardingService(database, config, GitManager())
    first_request = _request(
        request_id="concurrent-1",
        source_mode="init",
        relative_path="contended",
        project_id="concurrent-one",
        create_readme=False,
        create_github=False,
    )
    first = asyncio.create_task(first_service.onboard_project(first_request))
    await git.init_started.wait()

    with pytest.raises(ProjectOnboardingError) as locked:
        await second_service.onboard_project(
            _request(
                request_id="concurrent-2",
                source_mode="init",
                relative_path="contended",
                project_id="concurrent-two",
                create_readme=False,
                create_github=False,
            )
        )
    assert locked.value.code == "destination_locked"
    git.continue_init.set()
    assert (await first).project_id == "concurrent-one"
    assert (root / "contended").is_dir()


async def test_concurrent_replay_reports_phase_without_failing_active_request(onboarding):
    _, database, config, root, _ = onboarding
    git = _PausingGitManager()
    service = ProjectOnboardingService(database, config, git)
    request = _request(
        request_id="active-replay",
        source_mode="init",
        relative_path="active-replay",
        project_id="active-replay",
        create_readme=False,
        create_github=False,
    )
    active = asyncio.create_task(service.onboard_project(request))
    await git.init_started.wait()

    replay = await ProjectOnboardingService(database, config, GitManager()).onboard_project(
        request
    )

    assert replay.status == "running"
    assert replay.phase == "prepare"
    git.continue_init.set()
    completed = await active
    assert completed.project_id == "active-replay"
    record = await database.get_onboarding_request("active-replay")
    assert record is not None and record["status"] == "succeeded"
    assert (root / "active-replay").is_dir()


class _SimulatedCrash(BaseException):
    pass


async def test_reconstructed_service_resumes_request_owned_staging(onboarding, monkeypatch):
    service, database, config, root, _ = onboarding
    request = _request(
        request_id="recover-me",
        source_mode="init",
        relative_path="recovered",
        project_id="recovered",
        create_readme=False,
        create_github=False,
    )

    async def crash_before_publish(*_args, **_kwargs):
        raise _SimulatedCrash

    monkeypatch.setattr(service, "_publish_staging", crash_before_publish)
    with pytest.raises(_SimulatedCrash):
        await service.onboard_project(request)
    record = await database.get_onboarding_request("recover-me")
    assert record is not None and record["status"] == "pending"
    staging_paths = [
        Path(resource["path"])
        for resource in record["created_resources"]
        if resource["kind"] == "staging_directory"
    ]
    assert len(staging_paths) == 1 and staging_paths[0].is_dir()

    recovered = await ProjectOnboardingService(database, config, GitManager()).onboard_project(
        request
    )

    assert recovered.project_id == "recovered"
    assert (root / "recovered" / ".git").exists()
    assert not staging_paths[0].exists()


@pytest.mark.parametrize("source_mode", ["link", "init"])
async def test_registration_failure_rolls_back_only_request_owned_resources(
    onboarding, monkeypatch, source_mode
):
    service, database, _, root, _ = onboarding
    repo = _make_repo(root / "repo")
    before = _repository_snapshot(repo)

    async def fail_registration(*_args, **_kwargs):
        raise RuntimeError("database unavailable")

    monkeypatch.setattr(database, "register_onboarded_project", fail_registration)
    if source_mode == "link":
        request = _request(request_id="rollback-link")
        destination = repo
    else:
        request = _request(
            request_id="rollback-init",
            source_mode="init",
            relative_path="rollback-init",
            project_id="rollback-init",
            create_readme=True,
            create_github=False,
        )
        destination = root / "rollback-init"

    with pytest.raises(ProjectOnboardingError) as failure:
        await service.onboard_project(request)

    assert failure.value.code == "registration_failed"
    assert await database.get_project(request.project_id) is None
    assert await database.list_workspaces(request.project_id) == []
    if source_mode == "link":
        assert destination.is_dir()
        assert _repository_snapshot(repo) == before
    else:
        assert not destination.exists()
        assert not list(root.glob(".*aq-onboard*"))


async def test_status_maps_durable_terminal_record(onboarding):
    service, _, _, root, _ = onboarding
    _make_repo(root / "repo")
    result = await service.onboard_project(_request())

    status = await service.get_project_onboarding("request-1")

    assert status.request_id == "request-1"
    assert status.status == "completed"
    assert status.phase == "done"
    assert status.result == result


async def test_command_handler_delegates_onboard_and_status_to_service(onboarding):
    _, database, config, root, _ = onboarding
    _make_repo(root / "repo")
    orchestrator = Orchestrator(config)
    orchestrator.db = database
    handler = CommandHandler(orchestrator, config)
    payload = _request().model_dump(mode="json")

    result = await handler.execute("onboard_project", payload)
    status = await handler.execute(
        "get_project_onboarding", {"request_id": payload["request_id"]}
    )

    assert result["success"] is True
    assert result["project_id"] == "example-project"
    assert status["success"] is True
    assert status["status"] == "completed"
    assert status["result"]["workspace_id"] == result["workspace_id"]


@pytest.mark.parametrize(
    ("source", "clone_url"),
    [
        (
            {"github_repository": {"owner": "acme", "name": "widgets"}},
            "https://github.com/acme/widgets.git",
        ),
        (
            {"github_url": "https://github.com/acme/widgets"},
            "https://github.com/acme/widgets.git",
        ),
        (
            {"github_url": "git@github.com:acme/widgets.git"},
            "git@github.com:acme/widgets.git",
        ),
        (
            {"github_url": "acme/widgets"},
            "https://github.com/acme/widgets.git",
        ),
    ],
)
async def test_github_clone_normalizes_source_and_registers_real_clone(
    onboarding, tmp_path, source, clone_url
):
    _, database, config, root, _ = onboarding
    remote = _make_bare_remote(tmp_path / "clone-source.git")
    service = ProjectOnboardingService(
        database,
        config,
        _git_with_local_remote(remote),
    )
    request = _request(
        request_id="clone-request",
        source_mode="github_clone",
        relative_path="widgets",
        project_id="widgets",
        project_name="Widgets",
        **source,
    )

    result = await service.onboard_project(request)

    destination = root / "widgets"
    assert result.source_type == "clone"
    assert result.remote_url == clone_url
    assert result.default_branch == "trunk"
    assert result.actions == [
        "repository_cloned",
        "project_created",
        "workspace_registered",
        "vault_initialized",
    ]
    assert (destination / "from-remote.txt").read_text(encoding="utf-8") == "cloned\n"
    assert _git(destination, "remote", "get-url", "origin").stdout.strip() == clone_url
    project = await database.get_project("widgets")
    workspaces = await database.list_workspaces("widgets")
    assert project is not None and project.repo_url == clone_url
    assert len(workspaces) == 1 and workspaces[0].source_type is RepoSourceType.CLONE
    assert not list(root.glob(".*aq-onboard*"))


@pytest.mark.parametrize("owner_entry_kind", ["file", "symlink"])
async def test_github_clone_preserves_repository_owned_marker_path(
    onboarding, tmp_path, owner_entry_kind
):
    _, database, config, root, _ = onboarding
    victim = root / "victim"
    victim.write_text("untouched\n", encoding="utf-8")
    owner_entry = victim if owner_entry_kind == "symlink" else "tracked content\n"
    remote = _make_bare_remote(
        tmp_path / "hostile-source.git",
        owner_entry=owner_entry,
    )
    clone_url = "https://github.com/acme/hostile.git"
    service = ProjectOnboardingService(database, config, _git_with_local_remote(remote))

    await service.onboard_project(
        _request(
            request_id=f"hostile-{owner_entry_kind}",
            source_mode="github_clone",
            relative_path="hostile",
            project_id="hostile",
            github_url=clone_url,
        )
    )

    cloned_entry = root / "hostile" / ".aq-onboarding-owner"
    assert victim.read_text(encoding="utf-8") == "untouched\n"
    if owner_entry_kind == "symlink":
        assert cloned_entry.is_symlink()
        assert cloned_entry.readlink() == victim
    else:
        assert cloned_entry.read_text(encoding="utf-8") == "tracked content\n"


async def test_clone_retry_ledgers_published_directory_before_registration(
    onboarding, tmp_path, monkeypatch
):
    _, database, config, root, _ = onboarding
    remote = _make_bare_remote(tmp_path / "crash-source.git")
    git = _git_with_local_remote(remote)
    request = _request(
        request_id="clone-publish-crash",
        source_mode="github_clone",
        relative_path="widgets",
        project_id="widgets",
        github_url="https://github.com/acme/widgets",
    )
    append_resource = database.append_onboarding_resource

    async def crash_before_final_ledger(request_id, resource, **kwargs):
        if resource.get("kind") == "final_directory":
            raise _SimulatedCrash
        return await append_resource(request_id, resource, **kwargs)

    monkeypatch.setattr(database, "append_onboarding_resource", crash_before_final_ledger)
    with pytest.raises(_SimulatedCrash):
        await ProjectOnboardingService(database, config, git).onboard_project(request)

    destination = root / "widgets"
    assert destination.is_dir()
    assert list(root.glob(".*aq-onboard*"))
    record = await database.get_onboarding_request(request.request_id)
    assert record is not None
    assert not any(
        item["kind"] == "final_directory" for item in record["created_resources"]
    )

    monkeypatch.setattr(database, "append_onboarding_resource", append_resource)

    async def fail_registration(*_args, **_kwargs):
        raise RuntimeError("database unavailable")

    monkeypatch.setattr(database, "register_onboarded_project", fail_registration)
    with pytest.raises(ProjectOnboardingError) as failure:
        await ProjectOnboardingService(database, config, git).onboard_project(request)

    assert failure.value.code == "registration_failed"
    failed_record = await database.get_onboarding_request(request.request_id)
    assert failed_record is not None
    assert any(
        item["kind"] == "final_directory"
        for item in failed_record["created_resources"]
    )
    assert not destination.exists()
    assert not list(root.glob(".*aq-onboard*"))


async def test_final_directory_ledger_failure_removes_published_clone(
    onboarding, tmp_path, monkeypatch
):
    _, database, config, root, _ = onboarding
    remote = _make_bare_remote(tmp_path / "ledger-failure-source.git")
    request = _request(
        request_id="clone-final-ledger-failure",
        source_mode="github_clone",
        relative_path="widgets",
        project_id="widgets",
        github_url="https://github.com/acme/widgets",
    )
    append_resource = database.append_onboarding_resource

    async def fail_final_ledger(request_id, resource, **kwargs):
        if resource.get("kind") == "final_directory":
            raise RuntimeError("database unavailable after publish")
        return await append_resource(request_id, resource, **kwargs)

    monkeypatch.setattr(database, "append_onboarding_resource", fail_final_ledger)
    with pytest.raises(ProjectOnboardingError) as failure:
        await ProjectOnboardingService(
            database,
            config,
            _git_with_local_remote(remote),
        ).onboard_project(request)

    assert failure.value.code == "registration_failed"
    assert not (root / "widgets").exists()
    assert not list(root.glob(".*aq-onboard*"))


@pytest.mark.parametrize(("visibility", "flag"), [(None, "--private"), ("public", "--public")])
async def test_init_create_github_configures_origin_and_pushes_commit(
    onboarding, tmp_path, visibility, flag
):
    _, database, config, root, _ = onboarding
    github_url = "https://github.com/acme/widgets.git"
    remote = _make_bare_remote(tmp_path / "created.git", with_commit=False)
    gh, argv_log = _fake_gh(tmp_path)
    service = ProjectOnboardingService(
        database,
        config,
        _git_with_local_remote(remote),
        gh_client=gh,
    )
    values = {
        "request_id": f"create-{visibility or 'default'}",
        "source_mode": "init",
        "relative_path": "widgets",
        "project_id": "widgets",
        "project_name": "Widgets",
        "create_readme": True,
        "create_github": True,
        "github_owner": "acme",
        "github_repo": "widgets",
    }
    if visibility is not None:
        values["github_visibility"] = visibility

    result = await service.onboard_project(_request(**values))

    destination = root / "widgets"
    assert result.remote_url == github_url
    assert result.actions[:5] == [
        "repository_initialized",
        "readme_committed",
        "github_repository_created",
        "remote_configured",
        "branch_pushed",
    ]
    assert _git(destination, "remote", "get-url", "origin").stdout.strip() == github_url
    assert _git(remote, "rev-parse", "refs/heads/main").returncode == 0
    assert _gh_invocations(argv_log) == [["repo", "create", "acme/widgets", flag]]
    record = await database.get_onboarding_request(request_id := values["request_id"])
    assert request_id and record is not None
    assert {item["kind"] for item in record["created_resources"]} >= {
        "github_repository",
        "final_directory",
    }
    assert "ghp_" not in repr(record)


async def test_init_create_github_without_commit_configures_remote_without_push(
    onboarding, tmp_path
):
    _, database, config, root, _ = onboarding
    github_url = "https://github.com/acme/empty.git"
    remote = _make_bare_remote(tmp_path / "empty.git", with_commit=False)
    gh, _ = _fake_gh(tmp_path)
    service = ProjectOnboardingService(
        database,
        config,
        _git_with_local_remote(remote),
        gh_client=gh,
    )

    result = await service.onboard_project(
        _request(
            request_id="create-empty",
            source_mode="init",
            relative_path="empty",
            project_id="empty",
            project_name="Empty",
            create_readme=False,
            create_github=True,
            github_owner="acme",
        )
    )

    destination = root / "empty"
    assert result.remote_url == github_url
    assert "remote_configured" in result.actions
    assert "branch_pushed" not in result.actions
    assert _git(destination, "remote", "get-url", "origin").stdout.strip() == github_url
    assert _git(destination, "rev-parse", "HEAD", check=False).returncode != 0
    assert _git(remote, "show-ref", check=False).stdout == ""


async def test_create_github_rejects_option_like_default_branch_before_mutation(
    onboarding, tmp_path
):
    _, database, config, root, _ = onboarding
    gh, argv_log = _fake_gh(tmp_path)
    service = ProjectOnboardingService(database, config, GitManager(), gh_client=gh)

    with pytest.raises(ProjectOnboardingError) as failure:
        await service.onboard_project(
            _request(
                request_id="unsafe-branch",
                source_mode="init",
                relative_path="widgets",
                project_id="widgets",
                default_branch="--all",
                create_readme=True,
                create_github=True,
                github_owner="acme",
                github_repo="widgets",
            )
        )

    assert failure.value.code == "invalid_request"
    assert failure.value.field_errors == [
        {"field": "default_branch", "message": "invalid Git default branch name"}
    ]
    assert _gh_invocations(argv_log) == []
    assert list(root.iterdir()) == []


class _FailingGitManager(GitManager):
    def __init__(self, *, fail_command: str) -> None:
        super().__init__()
        self.fail_command = fail_command

    async def _arun(self, args, cwd=None, timeout=None):
        if args and args[0] == self.fail_command:
            raise GitError(CREDENTIALED_GIT_ERROR)
        return await super()._arun(args, cwd=cwd, timeout=timeout)

    async def acreate_checkout(self, repo_url: str, checkout_path: str) -> None:
        if self.fail_command == "clone":
            raise GitError(CREDENTIALED_GIT_ERROR)
        await super().acreate_checkout(repo_url, checkout_path)


async def test_clone_failure_scrubs_credentials_from_error_ledger_and_logs(
    onboarding, caplog
):
    _, database, config, root, _ = onboarding
    service = ProjectOnboardingService(database, config, _FailingGitManager(fail_command="clone"))

    with (
        caplog.at_level(logging.DEBUG),
        pytest.raises(ProjectOnboardingError) as failure,
    ):
        await service.onboard_project(
            _request(
                request_id="clone-secret",
                source_mode="github_clone",
                relative_path="widgets",
                project_id="widgets",
                github_url="https://github.com/acme/widgets",
            )
        )

    assert failure.value.code == "clone_failed"
    record = await database.get_onboarding_request("clone-secret")
    for surface in (
        repr(failure.value.to_dict()),
        repr(failure.value.__cause__),
        repr(record),
        *(entry.getMessage() for entry in caplog.records),
    ):
        assert "ABCDEFGHIJKLMNOPQRSTUVWXYZ" not in surface
        assert "alice:" not in surface
    assert not (root / "widgets").exists()
    assert not list(root.glob(".*aq-onboard*"))


async def test_push_failure_retains_remote_and_reports_safe_recovery(
    onboarding, tmp_path, caplog
):
    _, database, config, root, _ = onboarding
    gh, argv_log = _fake_gh(tmp_path)
    service = ProjectOnboardingService(
        database,
        config,
        _FailingGitManager(fail_command="push"),
        gh_client=gh,
    )

    with (
        caplog.at_level(logging.DEBUG),
        pytest.raises(ProjectOnboardingError) as failure,
    ):
        await service.onboard_project(
            _request(
                request_id="push-secret",
                source_mode="init",
                relative_path="widgets",
                project_id="widgets",
                create_readme=True,
                create_github=True,
                github_owner="acme",
                github_repo="widgets",
            )
        )

    assert failure.value.code == "push_failed"
    assert failure.value.details["github_repository_url"] == (
        "https://github.com/acme/widgets"
    )
    assert "retained" in failure.value.details["recovery_action"].lower()
    assert _gh_invocations(argv_log) == [["repo", "create", "acme/widgets", "--private"]]
    record = await database.get_onboarding_request("push-secret")
    assert record is not None
    assert any(item["kind"] == "github_repository" for item in record["created_resources"])
    for surface in (
        repr(failure.value.to_dict()),
        repr(failure.value.__cause__),
        repr(record),
        *(entry.getMessage() for entry in caplog.records),
    ):
        assert "ABCDEFGHIJKLMNOPQRSTUVWXYZ" not in surface
        assert "alice:" not in surface
    assert not (root / "widgets").exists()
    assert not list(root.glob(".*aq-onboard*"))


async def test_create_failure_scrubs_fake_gh_error_from_every_surface(
    onboarding, tmp_path, caplog
):
    _, database, config, root, _ = onboarding
    gh, _ = _fake_gh(tmp_path, fail_stderr=CREDENTIALED_GIT_ERROR)
    service = ProjectOnboardingService(database, config, GitManager(), gh_client=gh)

    with (
        caplog.at_level(logging.DEBUG),
        pytest.raises(ProjectOnboardingError) as failure,
    ):
        await service.onboard_project(
            _request(
                request_id="create-secret",
                source_mode="init",
                relative_path="widgets",
                project_id="widgets",
                create_readme=False,
                create_github=True,
                github_owner="acme",
                github_repo="widgets",
            )
        )

    assert failure.value.code == "github_repository_inaccessible"
    record = await database.get_onboarding_request("create-secret")
    for surface in (
        repr(failure.value.to_dict()),
        repr(failure.value.__cause__),
        repr(record),
        *(entry.getMessage() for entry in caplog.records),
    ):
        assert "ABCDEFGHIJKLMNOPQRSTUVWXYZ" not in surface
        assert "alice:" not in surface
    assert not (root / "widgets").exists()
    assert not list(root.glob(".*aq-onboard*"))


async def test_registration_failure_after_github_creation_retains_remote_url(
    onboarding, tmp_path, monkeypatch
):
    _, database, config, root, _ = onboarding
    remote = _make_bare_remote(tmp_path / "registration.git", with_commit=False)
    gh, argv_log = _fake_gh(tmp_path)
    service = ProjectOnboardingService(
        database,
        config,
        _git_with_local_remote(remote),
        gh_client=gh,
    )

    async def fail_registration(*_args, **_kwargs):
        raise RuntimeError("database unavailable")

    monkeypatch.setattr(database, "register_onboarded_project", fail_registration)
    with pytest.raises(ProjectOnboardingError) as failure:
        await service.onboard_project(
            _request(
                request_id="registration-github",
                source_mode="init",
                relative_path="widgets",
                project_id="widgets",
                create_readme=True,
                create_github=True,
                github_owner="acme",
                github_repo="widgets",
            )
        )

    assert failure.value.code == "registration_failed"
    assert failure.value.details["github_repository_url"] == (
        "https://github.com/acme/widgets"
    )
    assert "retained" in failure.value.details["recovery_action"].lower()
    assert _gh_invocations(argv_log) == [["repo", "create", "acme/widgets", "--private"]]
    record = await database.get_onboarding_request("registration-github")
    assert record is not None
    assert any(item["kind"] == "github_repository" for item in record["created_resources"])
    assert not (root / "widgets").exists()
    assert not list(root.glob(".*aq-onboard*"))


async def test_created_remote_is_reported_when_created_ledger_append_fails(
    onboarding, tmp_path, monkeypatch
):
    _, database, config, root, _ = onboarding
    gh, argv_log = _fake_gh(tmp_path)
    service = ProjectOnboardingService(database, config, GitManager(), gh_client=gh)
    append_resource = database.append_onboarding_resource

    async def fail_created_remote_ledger(request_id, resource, **kwargs):
        if resource.get("kind") == "github_repository":
            raise RuntimeError("database unavailable after GitHub creation")
        return await append_resource(request_id, resource, **kwargs)

    monkeypatch.setattr(database, "append_onboarding_resource", fail_created_remote_ledger)
    with pytest.raises(ProjectOnboardingError) as failure:
        await service.onboard_project(
            _request(
                request_id="created-ledger-failure",
                source_mode="init",
                relative_path="widgets",
                project_id="widgets",
                create_readme=False,
                create_github=True,
                github_owner="acme",
                github_repo="widgets",
            )
        )

    assert _gh_invocations(argv_log) == [["repo", "create", "acme/widgets", "--private"]]
    assert failure.value.code == "registration_failed"
    assert failure.value.details["github_repository_url"] == (
        "https://github.com/acme/widgets"
    )
    assert "retained" in failure.value.details["recovery_action"].lower()
    record = await database.get_onboarding_request("created-ledger-failure")
    assert record is not None
    assert any(
        item["kind"] == "github_repository_intent"
        and item["url"] == "https://github.com/acme/widgets"
        for item in record["created_resources"]
    )
    assert not (root / "widgets").exists()
    assert not list(root.glob(".*aq-onboard*"))


async def test_retry_after_crash_post_create_reports_retained_remote(
    onboarding, tmp_path, monkeypatch
):
    _, database, config, root, _ = onboarding
    gh, argv_log = _fake_gh(tmp_path)
    service = ProjectOnboardingService(database, config, GitManager(), gh_client=gh)
    append_resource = database.append_onboarding_resource

    async def crash_before_created_remote_ledger(request_id, resource, **kwargs):
        if resource.get("kind") == "github_repository":
            raise _SimulatedCrash
        return await append_resource(request_id, resource, **kwargs)

    monkeypatch.setattr(
        database,
        "append_onboarding_resource",
        crash_before_created_remote_ledger,
    )
    request = _request(
        request_id="created-ledger-crash",
        source_mode="init",
        relative_path="widgets",
        project_id="widgets",
        create_readme=False,
        create_github=True,
        github_owner="acme",
        github_repo="widgets",
    )
    with pytest.raises(_SimulatedCrash):
        await service.onboard_project(request)

    monkeypatch.setattr(database, "append_onboarding_resource", append_resource)
    assert gh._env is not None
    gh._env["FAKE_GH_FAIL_STDERR"] = (
        "GraphQL: Name already exists on this account (createRepository)"
    )
    with pytest.raises(ProjectOnboardingError) as failure:
        await ProjectOnboardingService(
            database,
            config,
            GitManager(),
            gh_client=gh,
        ).onboard_project(request)

    assert failure.value.code == "github_repository_conflict"
    assert failure.value.details["github_repository_url"] == (
        "https://github.com/acme/widgets"
    )
    assert "retained" in failure.value.details["recovery_action"].lower()
    assert _gh_invocations(argv_log) == [
        ["repo", "create", "acme/widgets", "--private"],
        ["repo", "create", "acme/widgets", "--private"],
    ]
    record = await database.get_onboarding_request("created-ledger-crash")
    assert record is not None
    assert any(
        item["kind"] == "github_repository_intent"
        and item["url"] == "https://github.com/acme/widgets"
        for item in record["created_resources"]
    )
    assert not (root / "widgets").exists()
    assert not list(root.glob(".*aq-onboard*"))


async def test_create_github_existing_name_maps_to_repository_conflict(onboarding, tmp_path):
    _, database, config, root, _ = onboarding
    gh, _ = _fake_gh(tmp_path)
    service = ProjectOnboardingService(database, config, GitManager(), gh_client=gh)

    with pytest.raises(ProjectOnboardingError) as failure:
        await service.onboard_project(
            _request(
                request_id="create-conflict",
                source_mode="init",
                relative_path="taken",
                project_id="taken",
                create_readme=False,
                create_github=True,
                github_owner="acme",
                github_repo="taken",
            )
        )

    assert failure.value.code == "github_repository_conflict"
    assert "github_repository_url" not in failure.value.details
    assert "recovery_action" not in failure.value.details
    assert not (root / "taken").exists()
    assert not list(root.glob(".*aq-onboard*"))
