...
def test_the_documented_steps_and_capabilities_match_the_registry():
    ...
    # Other adapters (the optional agent CLIs) register capabilities of their
    # own, so the invariant here is the database ones and the rule that every
    # capability the registry offers is named in the published surface.
    assert {CAPABILITY_MANAGED, CAPABILITY_ROTATE} <= set(registry.capabilities())
    for capability in registry.capabilities():
        assert capability in text, f"{capability} is not documented"
