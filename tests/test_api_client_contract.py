...
def test_generated_client_boilerplate_matches_what_the_pinned_generator_writes(tmp_path):
    ...
    generator version and on ``scripts/openapi-python-client.yaml`` (the two
    name overrides and the post hooks that format the generated Python) — so
    this generates them from an empty spec carrying the real ``info`` block
    (~0.5s) and compares them byte-for-byte, instead of running a full
    regeneration.
    """
