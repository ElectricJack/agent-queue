...
async def test_a_running_session_and_open_attempt_on_an_in_progress_task_is_returned(db):
    await seed_task_session_attempt(
        ...
        session_id="s1",
        now=NOW,
        session_started_at=NOW - 2 * 3600,
        attempt_started_at=NOW - 60,
    )
...
async def test_stale_session_activity_and_start_are_not_returned(db):
    ...
    await seed_task_session_attempt(
        ...
        session_id="s1",
        now=NOW,
        heartbeat_age=DEFAULT_STALE_AFTER + 100,
...
async def test_running_target_breaks_priority_and_start_ties_by_task_id(db):
    ...
    await seed_task_session_attempt(
        ...
        session_id="s-a",
        now=NOW,
        session_started_at=NOW - 90,
        attempt_started_at=NOW - 90,
        create_task=False,
    )
    await seed_task_session_attempt(
        db,
