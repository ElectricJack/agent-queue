...
async def test_batch_member_requires_append_only_review_evidence(db):
    ...
    async with db.immediate() as conn:
        ...
        await _integrity_error_in_savepoint(
            conn,
            insert(integration_batch_members).values(**member, review_evidence_id="missing-review"),
        )
        await conn.execute(insert(integration_review_evidence).values(**_review_evidence_values()))
        await conn.execute(
            insert(integration_batch_members).values(**member, review_evidence_id="review-1")
        )
...
async def test_active_batch_is_unique_per_project(db):
    ...
    async with db.immediate() as conn:
        ...
        await _integrity_error_in_savepoint(
            conn,
            insert(integration_batches).values(id="b2", **(values | {"request_id": "request-b2"})),
        )
...
async def test_sealed_batch_membership_is_immutable(db):
    ...
    async with db.immediate() as conn:
        ...
        await _integrity_error_in_savepoint(
            conn,
            insert(integration_batch_members).values(**(member | {"ordinal": 1, "task_id": "t2"})),
        )
        await _integrity_error_in_savepoint(
            conn,
            delete(integration_batch_members).where(integration_batch_members.c.batch_id == "b1"),
        )
...
async def test_prepared_intent_and_receipt_audit_are_immutable(db):
    ...
    async with db.immediate() as conn:
        ...
        await _integrity_error_in_savepoint(
            ...
            .where(integration_promotion_intents.c.id == "intent")
            .values(source_head="e" * 40),
        )
        await _integrity_error_in_savepoint(
            conn,
            update(integration_promotion_intents)
            .where(integration_promotion_intents.c.id == "intent")
            .values(fence_token=2),
        )
        await conn.execute(
            update(integration_promotion_intents)
            .where(integration_promotion_intents.c.id == "intent")
        ...
        await _integrity_error_in_savepoint(
            ...
            .where(task_delivery_receipts.c.id == "receipt")
            .values(after_sha="e" * 40),
        )
        await _integrity_error_in_savepoint(
...
async def test_only_one_active_repair_operation_can_target_a_parent(db):
    ...
    async with db.immediate() as conn:
        ...
        await conn.execute(
            insert(integration_repair_operations).values(id="op1", episode_id="one", **values)
        )
        await _integrity_error_in_savepoint(
    ...
    async with db.immediate() as conn:
        ...
        await conn.execute(
            insert(task_delivery_receipts).values(
                id="receipt",
                domain_key="receipt-key",
                repository_id="repo",
                target_branch="parent",
                disposition="noop",
                resolution_evidence={"reason": "already-delivered"},
                created_at=1.0,
            )
        )
        await conn.execute(insert(integration_review_evidence).values(**_review_evidence_values()))
...
async def test_upgrade_from_prior_schema_creates_every_integration_table(
    ...
    try:
        ...
        try:
            ...
            async with engine.connect() as conn:
                assert (
                    await conn.execute(
                        text("SELECT to_regclass('integration_candidate_member_results')")
                    )
                ).scalar_one()
                assert (
                    await conn.execute(
...
def test_sqlite_task8a_migration_cycle_from_previous_head(tmp_path):
    ...
    try:
        ...
        member_columns = {
            column["name"] for column in inspect(engine).get_columns("integration_batch_members")
        }
        ...
        with engine.connect() as conn:
            ...
            assert migrated["review_evidence_id"] == ("task8a-legacy-review:legacy-batch:0")
...
def test_sqlite_task8a_upgrade_refuses_legacy_null_batch_identity_before_ddl(
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            assert (
                tuple(column["name"] for column in inspect(conn).get_columns("integration_batches"))
                == columns_before
            )
            assert (
                tuple(
        ...
        with engine.begin() as conn:
            conn.execute(text("DELETE FROM integration_batches WHERE id = 'legacy-null'"))
        migrate("head")
        migrate("c7d8e9f0a1b2", downgrade=True)
        migrate("head")
        assert "request_id" in {
            column["name"] for column in inspect(engine).get_columns("integration_batches")
        }
    finally:
        engine.dispose()
...
async def test_postgres_task8a_upgrade_refuses_legacy_null_batch_identity_before_ddl(
    ...
    try:
        ...
        try:
            ...
            async with engine.connect() as conn:
                columns_before = await conn.run_sync(
                    lambda sync_conn: tuple(
                        column["name"]
                        for column in inspect(sync_conn).get_columns("integration_batches")
                    )
                )
                guards_before = tuple(
                    (
            ...
            async with engine.begin() as conn:
                await conn.execute(text("DELETE FROM integration_batches WHERE id = 'legacy-null'"))
            await migrate("head")
