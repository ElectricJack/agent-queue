...
# Clear of tests/db_fixtures.py's template-build key (_TEMPLATE_LOCK_KEY).
...
async def dispose_owned_databases() -> None:
    """Drop only databases created by this process, all at once, then release its owner lock.

    Drops are issued in reverse creation order, so scratch children go before
    the process's worker database, but none waits for another to finish. The
    owner lock is released only after every drop has finished or hit the
    deadline, so no sweep can take a database this process is still dropping.
    """
    global _OWNER
