...
(`docs/specs/provider-failover.md` D23).  No `prova` binary is ever run:
...
`prova` slice of an intelligence class, and its availability key is its id.
...
```json
{
  ...
  "dialogs": [
    {
      "name": "login-required",
      "pattern": "Please log in to prova",
      "keys": [],
      "quarantine": true,
      "signal": "auth"
    },
    {
      "name": "usage-limit",
      "pattern": "You've hit your prova usage limit",
      "keys": [],
      "quarantine": true,
      "signal": "usage"
    }
