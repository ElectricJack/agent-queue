...
from src.database.tables import (integration_batches, integration_candidate_revisions,
    integration_repair_operations, integration_repair_stages, integration_candidate_publications,
...
@pytest.mark.parametrize("case", ["exact", "missing", "remote", "old", "branch", "episode", "reserved", "ref_published"])
async def test_resume_only_metadata_after_exact_applied_ref(db, case):  # noqa: F811
    ...
    async with db.immediate() as conn:
        ...
        if state == "pr_reserved":
            await conn.execute(update(integration_candidate_publications).values(state="pr_reserved"))
        if case != "missing":
            await conn.execute(insert(integration_candidate_ref_mutations).values(
                id="publication-mutation", batch_id="human-batch", revision=0,
                purpose="candidate_final", repository_id="repo",
                branch="refs/heads/aq/integration/human",
                target_branch="refs/heads/aq/integration/other" if case == "branch" else "refs/heads/aq/integration/human",
                expected_old_sha=("d" if case == "old" else "b") * 40,
                desired_sha=("d" if case == "remote" else "c") * 40, remote_sha=("d" if case == "remote" else "c") * 40,
                operation_id="human-operation", operation_episode_id="wrong" if case == "episode" else "episode",
                operation_stage=0, lease_owner_id="lease", lease_fence_token=1,
                branch_owner_id="collector", branch_owner_role="collector", branch_fence_token=1,
                nonce="nonce", state="applied", expires_at=55, created_at=50, updated_at=50,
            ))
    ...
    async with db._engine.connect() as conn:
        assert (await conn.execute(select(integration_candidate_publications.c.state))).scalar_one() == state
